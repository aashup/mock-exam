...
set_error_handler(function ($Bc, $Dc) {
    return (bool) preg_match('~^Undefined (array key|offset|index)~', $Dc);
}, E_WARNING | E_NOTICE);
...
}function remove_slashes(array $Jj, $Zc = false)
{
    $J = [];
    foreach ($Jj as $x => $X) {
        $J[stripslashes($x)] = (is_array($X) ? remove_slashes($X, $Zc) : ($Zc ? $X : stripslashes($X)));
    }

    return $J;
}function bracket_escape($u, $Ca = false)
{
    static $cj = [':' => ':1', ']' => ':2', '[' => ':3', '"' => ':4'];

    return strtr($u, ($Ca ? array_flip($cj) : $cj));
}function min_version($Mj, $We = '', $g = null)
{
    $g = connection($g);
    $Vh = $g->server_info;
...
}function sid()
{
    ...
    return $J;
}function set_password($Lj, $N, $V, $F)
{
    $_SESSION['pwds'][$Lj][$N][$V] = ($_COOKIE['adminer_key'] && is_string($F) ? [encrypt_string($F, $_COOKIE['adminer_key'])] : $F);
}function get_password()
{
    ...
    return $J;
}function get_val($H, $m = 0, $tb = null)
{
    $tb = connection($tb);
    $I = $tb->query($H);
    if (! is_object($I)) {
...
}function get_vals($H, $d = 0)
{
    ...
    return $J;
}function get_key_vals($H, $g = null, $Yh = true)
{
    $g = connection($g);
    $J = [];
    $I = $g->query($H);
    ...
    return $J;
}function get_rows($H, $g = null, $l = "<p class='error'>")
{
    $tb = connection($g);
    $J = [];
    $I = $tb->query($H);
    ...
    return $J;
}function unique_array($K, array $w)
{
    foreach ($w as $v) {
        if (preg_match('~PRIMARY|UNIQUE~', $v['type']) && ! $v['partial']) {
            $J = [];
...
}function where(array $Z, array $n = [])
{
    ...
    foreach ((array) $Z['where'] as $x => $X) {
        ...
        $m = idx($n, $x, []);
        $Wc = $m['type'];
        $J[] = $d.(JUSH == 'sql' && $Wc == 'json' ? ' = CAST('.q($X).' AS JSON)' : (JUSH == 'pgsql' && preg_match('~^json~', $Wc) ? '::jsonb = '.q($X).'::jsonb' : (JUSH == 'sql' && is_numeric($X) && preg_match('~\.~', $X) ? ' LIKE '.q($X) : (JUSH == 'mssql' && strpos($Wc, 'datetime') === false ? ' LIKE '.q(preg_replace('~[_%[]~', '[\0]', $X)) : ' = '.unconvert_field($m, q($X))))));
        if (JUSH == 'sql' && preg_match('~char|text~', $Wc) && preg_match('~[^ -@]~', $X)) {
...
}function where_check($X, array $n = [])
{
    ...
    return where($Wa, $n);
}function where_link($s, $d, $Y, $Yf = '=')
{
    return "&where%5B$s%5D%5Bcol%5D=".urlencode($d)."&where%5B$s%5D%5Bop%5D=".urlencode(($Y !== null ? $Yf : 'IS NULL'))."&where%5B$s%5D%5Bval%5D=".urlencode($Y);
}function convert_fields(array $e, array $n, array $M = [])
{
    $J = '';
    foreach ($e as $x => $X) {
    ...
    return $J;
}function cookie($B, $Y, $Pe = 2592000)
{
    header("Set-Cookie: $B=".rawurlencode($Y).($Pe ? '; expires='.gmdate('D, d M Y H:i:s', time() + $Pe).' GMT' : '').'; path='.preg_replace('~\?.*~', '', $_SERVER['REQUEST_URI']).(HTTPS ? '; secure' : '').'; HttpOnly; SameSite=lax', false);
}function get_settings($Bb)
{
    parse_str($_COOKIE[$Bb], $Zh);

    return $Zh;
}function get_setting($x, $Bb = 'adminer_settings', $k = null)
{
    return idx(get_settings($Bb), $x, $k);
}function save_settings(array $Zh, $Bb = 'adminer_settings')
{
...
}function stop_session($hd = false)
{
...
}function auth_url($Lj, $N, $V, $j = null)
{
    $zj = remove_from_uri(implode('|', array_keys(SqlDriver::$drivers)).'|username|ext|'.($j !== null ? 'db|' : '').($Lj == 'mssql' || $Lj == 'pgsql' ? '' : 'ns|').session_name());
    preg_match('~([^?]*)\??(.*)~', $zj, $A);

    return "$A[1]?".(sid() ? SID.'&' : '').($Lj != 'server' || $N != '' ? urlencode($Lj).'='.urlencode($N).'&' : '').($_GET['ext'] ? 'ext='.urlencode($_GET['ext']).'&' : '').'username='.urlencode($V).($j != '' ? '&db='.urlencode($j) : '').($A[2] ? "&$A[2]" : '');
}function is_ajax()
{
    return $_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest';
}function redirect($Se, $mf = null)
{
    if ($mf !== null) {
        restart_session();
        $_SESSION['messages'][preg_replace('~^[^?]*~', '', ($Se !== null ? $Se : $_SERVER['REQUEST_URI']))][] = $mf;
    }if ($Se !== null) {
        if ($Se == '') {
            $Se = '.';
        }header("Location: $Se");
...
}function query_redirect($H, $Se, $mf, $jh = true, $Ic = true, $Rc = false, $Pi = '')
{
    if ($Ic) {
        $oi = microtime(true);
        $Rc = ! connection()->query($H);
        $Pi = format_time($oi);
    }$ii = ($H ? adminer()->messageQuery($H, $Pi, $Rc) : '');
    if ($Rc) {
        adminer()->error
...
}function apply_queries($H, array $T, $Ec = 'Adminer\table')
{
    foreach ($T as $R) {
        if (! queries("$H ".$Ec($R))) {
            return false;
    ...
    return true;
}function queries_redirect($Se, $mf, $jh)
{
    $eh = implode("\n", Queries::$queries);
    $Pi = format_time(Queries::$start);

    return query_redirect($eh, $Se, $mf, $jh, false, ! $jh, $Pi);
}function format_time($oi)
{
    return sprintf('%.3f s', max(0, microtime(true) - $oi));
}function relative_uri()
{
    return str_replace(':', '%3a', preg_replace('~^[^?]*/([^?]*)~', '\1', $_SERVER['REQUEST_URI']));
}function remove_from_uri($vg = '')
{
    return substr(preg_replace("~(?<=[?&])($vg".(SID ? '' : '|'.session_name()).')=[^&]*&~', '', relative_uri().'&'), 0, -1);
}function get_file($x, $Rb = false, $Xb = '')
{
    $Yc = $_FILES[$x];
    if (! $Yc) {
        return null;
...
}function repeat_pattern($Hg, $y)
{
    return str_repeat("$Hg{0,65535}", $y / 65535)."$Hg{0,".($y % 65535).'}';
}function is_utf8($X)
{
...
}function table_status1($R, $Sc = false)
{
    $J = table_status($R, $Sc);
...
}function fields_from_edit()
{
    ...
    foreach ((array) $_POST['field_keys'] as $x => $X) {
        ...
        $J[$B] = ['field' => $B, 'privileges' => ['insert' => 1, 'update' => 1, 'where' => 1, 'order' => 1], 'null' => 1, 'auto_increment' => ($x == driver()->primary)];
    }

    return $J;
}function dump_headers($Pd, $xf = false)
{
    $J = adminer()->dumpHeaders($Pd, $xf);
    $rg = $_POST['output'];
    if ($rg != 'text') {
        header('Content-Disposition: attachment; filename='.adminer()->dumpFilename($Pd).".$J".($rg != 'file' && preg_match('~^[0-9a-z]+$~', $rg) ? ".$rg" : ''));
    }session_write_close();
    if (! ob_get_level()) {
        ob_start(null, 4096);
    }ob_flush();
...
}function dump_csv(array $K)
{
    ...
    }echo implode(($_POST['format'] == 'csv' ? ',' : ($lj ? "\t" : ';')), $K)."\r\n";
...
}function file_open_lock($o)
{
    ...
    return $q;
}function file_write_unlock($q, $Lb)
{
    rewind($q);
    fwrite($q, $Lb);
...
}function select_value($X, $_, array $m, $Oi)
{
    if (is_array($X)) {
        $J = '';
        if (array_filter($X, 'is_array') == array_values($X)) {
            $Ce = [];
            foreach ($X as $W) {
                $Ce += array_fill_keys(array_keys($W), null);
            }foreach (array_keys($Ce) as $Ae) {
                $J
                .= '<th>'.h($Ae);
            }foreach ($X as $W) {
                $J
                .= '<tr>';
                foreach (array_merge($Ce, $W) as $Gj) {
                    $J
                    .= '<td>'.select_value($Gj, $_, $m, $Oi);
                }
            }
        } else {
            foreach ($X as $Ae => $W) {
    ...
    }$J = adminer()->editVal(driver()->value($X, $m), $m);
    if ($J !== null) {
        if (! is_utf8($J)) {
            $J = "\0";
    ...
    return adminer()->selectVal($J, $_, $m, $X);
}function is_blob(array $m)
{
    return preg_match('~blob|bytea|raw|file~', $m['type']) && ! in_array($m['type'], idx(driver()->structuredTypes(), 'User types', []));
}function is_mail($sc)
{
    $xa = '[-a-z0-9!#$%&\'*+/=?^_`{|}~]';
    ...
    return is_string($sc) && preg_match("(^$Hg(,\\s*$Hg)*\$)i", $sc);
}function is_url($Q)
{
    $fc = '[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';

    return preg_match("~^((https?):)?//($fc?\\.)+$fc(:\\d+)?(/.*)?(\\?.*)?(#.*)?\$~i", $Q);
}function is_shortable(array $m)
{
    return ! preg_match('~'.number_type().'|date|time|year~', $m['type']);
}function host_port($N)
{
    return preg_match('~^(\[(.+)]|([^:]+)):([^:]+)$~', $N, $A) ? [$A[2].$A[3], $A[4]] : [$N, ''];
}function count_rows($R, array $Z, $te, array $vd)
{
    $H = ' FROM '.table($R).($Z ? ' WHERE '.implode(' AND ', $Z) : '');

    return $te && (JUSH == 'sql' || count($vd) == 1) ? 'SELECT COUNT(DISTINCT '.implode(', ', $vd).")$H" : 'SELECT COUNT(*)'.($te ? " FROM (SELECT 1$H GROUP BY ".implode(', ', $vd).') x' : $H);
}function slow_query($H)
{
    ...
    if (! $di && support('kill')) {
        ...
        if ($g && ($j == '' || $g->select_db($j))) {
            ...
            echo script("const timeout = setTimeout(() => { ajax('".js_escape(ME)."script=kill', function () {}, 'kill=$De&token=".get_token()."'); }, 1000 * $Qi);");
        }
    }ob_flush();
    flush();
...
{
    [$Yi, $hh] = explode(':', $_POST['token']);

    return ($hh ^ $_SESSION['token']) == $Yi;
}function lzw_decompress($Ia)
{
    $cc = 256;
    $Ja = 8;
    $gb = [];
    ...
    return $J;
}function script($fi, $bj = "\n")
{
    return '<script'.nonce().">$fi</script>$bj";
}function script_src($_j, $Ub = false)
{
...
{
    return str_replace("\n", '<br>', $Q);
}function checkbox($B, $Y, $Za, $Fe = '', $Xf = '', $db = '', $He = '')
{
    $J = "<input type='checkbox' name='$B' value='".h($Y)."'".($Za ? ' checked' : '').($He ? " aria-labelledby='$He'" : '').'>'.($Xf ? script("qsl('input').onclick = function () { $Xf };", '') : '');

    return $Fe != '' || $db ? '<label'.($db ? " class='$db'" : '').">$J".h($Fe).'</label>' : $J;
}function optionlist($cg, $Nh = null, $Dj = false)
{
    $J = '';
    foreach ($cg as $Ae => $W) {
        $dg = [$Ae => $W];
    ...
    return $J;
}function html_select($B, array $cg, $Y = '', $Wf = '', $He = '')
{
    static $Fe = 0;
    $Ge = '';
    if (! $He && substr($cg[''], 0, 1) == '(') {
        $Fe++;
        $He = "label-$Fe";
        $Ge = "<option value='' id='$He'>".h($cg['']);
        unset($cg['']);
    }

    return "<select name='".h($B)."'".($He ? " aria-labelledby='$He'" : '').'>'.$Ge.optionlist($cg, $Y).'</select>'.($Wf ? script("qsl('select').onchange = function () { $Wf };", '') : '');
}function html_radios($B, array $cg, $Y = '', $Rh = '')
{
    $J = '';
    foreach ($cg as $x => $X) {
        $J
    ...
    return $J;
}function confirm($mf = '', $Oh = "qsl('input')")
{
    return script("$Oh.onclick = () => confirm('".($mf ? js_escape($mf) : 'Are you sure?')."');", '');
}function print_fieldset($t, $Ne, $Pj = false)
{
    echo '<fieldset><legend>',"<a href='#fieldset-$t'>$Ne</a>",script("qsl('a').onclick = partial(toggle, 'fieldset-$t');", ''),'</legend>',"<div id='fieldset-$t'".($Pj ? '' : " class='hidden'").">\n";
}function bold($La, $db = '')
{
...
}function file_input($le)
{
    ...
    return ini_bool('file_uploads') ? $le.script("qsl('input[type=\"file\"]').onchange = partialArg(fileChange, "."$cf, '".sprintf('Increase %s.', "$bf = $cf")."', ".ini_bytes('upload_max_filesize').", '".sprintf('Increase %s.', "$xj = $yj")."')") : 'File uploads are disabled.';
}function enum_input($U, $ya, array $m, $Y, $vc = '')
{
    preg_match_all("~'((?:[^']|'')*)'~", $m['length'], $Ze);
    $Tg = ($m['type'] == 'enum' ? 'val-' : '');
    $Za = (is_array($Y) ? in_array('null', $Y) : $Y === null);
    $J = ($m['null'] && $Tg ? "<label><input type='$U'$ya value='null'".($Za ? ' checked' : '')."><i>$vc</i></label>" : '');
    foreach ($Ze[1] as $X) {
        $X = stripcslashes(str_replace("''", "'", $X));
        $Za = (is_array($Y) ? in_array($Tg.$X, $Y) : $Y === $X);
        $J
        .= " <label><input type='$U'$ya value='".h($Tg.$X)."'".($Za ? ' checked' : '').'>'.h(adminer()->editVal($X, $m)).'</label>';
    }

    return $J;
}function input(array $m, $Y, $r, $Ba = false)
{
    $B = h(bracket_escape($m['field']));
    echo "<td class='function'>";
    if (is_array($Y) && ! $r) {
        $r = 'json';
    }$ze = ($r == 'json' || preg_match('~^jsonb?$~', $m['type']));
    if ($ze && $Y != '' && (JUSH != 'pgsql' || $m['type'] != 'json')) {
        $Y = json_encode(is_array($Y) ? $Y : json_decode($Y), 128 | 64 | 256);
    }$th = (JUSH == 'mssql' && $m['auto_increment']);
    if ($th && ! $_POST['save']) {
        $r = null;
    }$qd = (isset($_GET['select']) || $th ? ['orig' => 'original'] : []) + adminer()->editFunctions($m);
    $Ac = driver()->enumLength($m);
    if ($Ac) {
        $m['type'] = 'enum';
        $m['length'] = $Ac;
    }$ya = " name='fields[$B]".($m['type'] == 'enum' || $m['type'] == 'set' ? '[]' : '')."'".($Ba ? ' autofocus' : '');
    echo driver()->unconvertFunction($m).' ';
    $R = $_GET['edit'] ?: $_GET['select'];
    if ($m['type'] == 'enum') {
        echo h($qd['']).'<td>'.adminer()->editInput($R, $m, $ya, $Y);
    } else {
        $Cd = (in_array($r, $qd) || isset($qd[$r]));
        echo (count($qd) > 1 ? "<select name='function[$B]'>".optionlist($qd, $r === null || $Cd ? $r : '').'</select>'.on_help("event.target.value.replace(/^SQL\$/, '')", 1).script("qsl('select').onchange = functionChange;", '') : h(reset($qd))).'<td>';
        $le = adminer()->editInput($R, $m, $ya, $Y);
        if ($le != '') {
            echo $le;
            ...
            echo enum_input('checkbox', $ya, $m, (is_string($Y) ? explode(',', $Y) : $Y));
            ...
            if (JUSH == 'sql' && min_version(5.6) && preg_match('~time~', $m['type'])) {
                $if += 7;
            }echo '<input'.((! $Cd || $r === '') && preg_match('~(?<!o)int(?!er)~', $m['type']) && ! preg_match('~\[\]~', $m['full_type']) ? " type='number'" : '')." value='".h($Y)."'".($if ? " data-maxlength='$if'" : '').(preg_match('~char|binary~', $m['type']) && $if > 20 ? " size='".($if > 99 ? 60 : 40)."'" : '')."$ya>";
        ...
        foreach ($qd as $x => $X) {
            ...
            echo script("qsl('td').oninput = partial(skipOriginal, $ad);");
        }
    }
}function process_input(array $m)
{
    ...
    if ($Y === null) {
        ...
        $Yc = get_file("fields-$u");
        if (! is_string($Yc)) {
            return false;
        }

        return driver()->quoteBinary($Yc);
    }
...
}function search_tables()
{
    ...
    foreach (table_status('', true) as $R => $S) {
        ...
        if (isset($S['Engine']) && $B != '' && (! $_POST['tables'] || in_array($R, $_POST['tables']))) {
            ...
            if (! $I || $I->fetch_row()) {
                $Xg = "<a href='".h(ME.'select='.urlencode($R).'&where[0][op]='.urlencode($_GET['where'][0]['op']).'&where[0][val]='.urlencode($_GET['where'][0]['val']))."'>$B</a>";
                echo "$Qh<li>".($I ? $Xg : "<p class='error'>$Xg: ".error())."\n";
                $Qh = '';
            }
        }
    }echo ($Qh ? "<p class='message'>".'No tables.' : '</ul>')."\n";
}function on_help($mb, $bi = 0)
{
    return script("mixin(qsl('select, input'), {onmouseover: function (event) { helpMouseover.call(this, event, $mb, $bi) }, onmouseout: helpMouseout});", '');
}function edit_form($R, array $n, $K, $wj, $l = '')
{
    $_i = adminer()->tableName(table_status1($R, true));
    page_header(($wj ? 'Edit' : 'Insert'), $l, ['select' => [$R, $_i]], $_i);
    ...
    if (! $n) {
        ...
        foreach ($n as $B => $m) {
            ...
            }if (($wj && ! isset($m['privileges']['update'])) || $m['generated']) {
                echo "<td class='function'><td>".select_value($Y, '', $m, null);
            } else {
                $pc = true;
...
}function shorten_utf8($Q, $y = 80, $ui = '')
{
    if (! preg_match('(^('.repeat_pattern("[\t\r\n -\x{10FFFF}]", $y).')($)?)u', $Q, $A)) {
        preg_match('(^('.repeat_pattern("[\t\r\n -~]", $y).')($)?)', $Q, $A);
    }

    return h($A[1]).$ui.(isset($A[2]) ? '' : '<i>â€¦</i>');
}function icon($Od, $B, $Nd, $Si)
{
    return "<button type='submit' name='$B' title='".h($Si)."' class='icon icon-$Od'><span>$Nd</span></button>";
}if (isset($_GET['file'])) {
...
if (! defined('SID')) {
    ...
    $_GET = remove_slashes($_GET, $Zc);
    $_POST = remove_slashes($_POST, $Zc);
    $_COOKIE = remove_slashes($_COOKIE, $Zc);
}if (function_exists('get_magic_quotes_runtime') && get_magic_quotes_runtime()) {
    set_magic_quotes_runtime(false);
}@set_time_limit(0);
@ini_set('precision', '15');
function lang($u, $Kf = null)
{
    $ua = func_get_args();
    $ua[0] = $u;

    return call_user_func_array('Adminer\lang_format', $ua);
}function lang_format($dj, $Kf = null)
{
    if (is_array($dj)) {
        $Og = ($Kf == 1 ? 0 : 1);
        $dj = $dj[$Og];
    ...
    $ua = func_get_args();
    array_shift($ua);
    $ld = str_replace('%d', '%s', $dj);
    if ($ld != $dj) {
        $ua[0] = format_number($Kf);
    }

    return vsprintf($ld, $ua);
...
abstract class SqlDb
{
    ...
    abstract public function select_db($Nb);

    abstract public function query($H, $oj = false);
    ...
    abstract class PdoDb extends SqlDb
    {
        ...
        public function dsn($mc, $V, $F, array $cg = [])
        {
            $cg[\PDO::ATTR_ERRMODE] = \PDO::ERRMODE_SILENT;
            $cg[\PDO::ATTR_STATEMENT_CLASS] = ['Adminer\PdoResult'];
            try {
                $this->pdo = new \PDO($mc, $V, $F, $cg);
            } catch (\Exception$Gc) {
        ...
        public function query($H, $oj = false)
        {
            $I = $this->pdo->query($H);
            $this->error = '';
            if (! $I) {
    ...
    }class PdoResult extends \PDOStatement
    {
        ...
        private function fetch_array($uf)
        {
            $J = $this->fetch($uf);
...
}abstract class SqlDriver
{
    ...
    public function select($R, array $M, array $Z, array $vd, array $eg = [], $z = 1, $D = 0, $Xg = false)
    {
        $te = (count($vd) < count($M));
        $H = adminer()->selectQueryBuild($M, $Z, $vd, $eg, $z, $D);
        if (! $H) {
            $H = 'SELECT'.limit(($_GET['page'] != 'last' && $z && $vd && $te && JUSH == 'sql' ? 'SQL_CALC_FOUND_ROWS ' : '').implode(', ', $M)."\nFROM ".table($R), ($Z ? "\nWHERE ".implode(' AND ', $Z) : '').($vd && $te ? "\nGROUP BY ".implode(', ', $vd) : '').($eg ? "\nORDER BY ".implode(', ', $eg) : ''), $z, ($D ? $z * $D : 0), "\n");
        }$oi = microtime(true);
        $J = $this->conn->query($H);
        if ($Xg) {
            echo adminer()->selectQuery($H, $oi, ! $J);
        }

        return $J;
    }

    public function delete($R, $fh, $z = 0)
    {
        $H = 'FROM '.table($R);

        return queries('DELETE'.($z ? limit1($R, $H, $fh) : " $H$fh"));
    }

    public function update($R, array $O, $fh, $z = 0, $Rh = "\n")
    {
        $Jj = [];
        foreach ($O as $x => $X) {
            $Jj[] = "$x = $X";
        }$H = table($R)." SET$Rh".implode(",$Rh", $Jj);

        return queries('UPDATE'.($z ? limit1($R, $H, $fh, $Rh) : " $H$fh"));
    }
    ...
    public function slowQuery($H, $Qi) {}

    public function convertSearch($u, array $X, array $m)
    {
        return $u;
    }
    ...
    public function tableHelp($B, $xe = false) {}
    ...
    public function indexAlgorithms(array $yi)
    {
        return [];
    }
...
if (isset($_GET['sqlite'])) {
    ...
    if (class_exists('SQLite3') && $_GET['ext'] != 'pdo') {
        abstract class SqliteDb extends SqlDb
        {
            ...
            public function attach($o, $V, $F)
            {
                $this->link = new \SQLite3($o);
                $Mj = $this->link->version();
                $this->server_info = $Mj['versionString'];
    ...
    }class Driver extends SqlDriver
    {
        ...
        public function checkConstraints($R)
        {
            ...
            return array_combine($Ze[2], $Ze[2]);
        }
    ...
    {
        return idf_escape($u);
    }function get_databases($gd)
    {
        return [];
    }function limit($H, $Z, $z, $C = 0, $Rh = ' ')
    {
        return " $H$Z".($z ? $Rh."LIMIT $z".($C ? " OFFSET $C" : '') : '');
    }function limit1($R, $H, $Z, $Rh = "\n")
    {
    ...
    }function fields($R)
    {
        ...
        foreach (get_rows('PRAGMA table_'.(min_version(3.31) ? 'x' : '').'info('.table($R).')') as $K) {
            ...
            $U = strtolower($K['type']);
            $k = $K['dflt_value'];
            $J[$B] = ['field' => $B, 'type' => (preg_match('~int~i', $U) ? 'integer' : (preg_match('~char|clob|text~i', $U) ? 'text' : (preg_match('~blob~i', $U) ? 'blob' : (preg_match('~real|floa|doub~i', $U) ? 'real' : 'numeric')))), 'full_type' => $U, 'default' => (preg_match("~^'(.*)'$~", $k, $A) ? str_replace("''", "'", $A[1]) : ($k == 'NULL' ? null : $k)), 'null' => ! $K['notnull'], 'privileges' => ['select' => 1, 'insert' => 1, 'update' => 1, 'where' => 1, 'order' => 1], 'primary' => $K['pk']];
            if ($K['pk']) {
                if ($G != '') {
    ...
    }function check_sqlite_name($B)
    {
        ...
        if (! preg_match("~^[^\\0]*\\.($Oc)\$~", $B)) {
            connection()->error = sprintf('Please use one of the extensions %s.', str_replace('|', ', ', $Oc));

            return false;
        }

        return true;
    }function create_database($j, $c)
    {
        if (file_exists($j)) {
            ...
            connection()->error = $Gc->getMessage();
    ...
    }function alter_table($R, $B, $n, $id, $ob, $wc, $c, $_a, $E)
    {
        $Bj = ($R == '' || $id);
        foreach ($n as $m) {
            if ($m[0] != '' || ! $m[1] || $m[2]) {
        ...
        } elseif (! recreate_table($R, $B, $b, $pg, $id, $_a)) {
            return false;
        }if ($_a) {
            queries('BEGIN');
        ...
        return true;
    }function recreate_table($R, $B, array $n, array $pg, array $id, $_a = '', $w = [], $ic = '', $ja = '')
    {
        if ($R != '') {
            if (! $n) {
            ...
            foreach ($w as $x => $X) {
                ...
                foreach ($v['columns'] as $x => $d) {
                    if (! $pg[$d]) {
                        continue 2;
                    }$e[] = $pg[$d].($v['descs'][$x] ? ' DESC' : '');
                }if (! $kc[$Be]) {
                    if ($v['type'] != 'PRIMARY' || ! $Wg) {
                        $w[] = [$v['type'], $Be, $e];
                    }
                }
            }foreach ($w as $x => $X) {
                ...
                foreach ($p['source'] as $x => $d) {
                    if (! $pg[$d]) {
                        continue 2;
                    }$p['source'][$x] = idf_unescape($pg[$d]);
                }if (! isset($id[" $Be"])) {
                    $id[] = ' '.format_foreign_key($p);
                }
            }queries('BEGIN');
        }$Ua = [];
        foreach ($n as $m) {
            if (preg_match('~GENERATED~', $m[3])) {
                unset($pg[array_search($m[0], $pg)]);
            }$Ua[] = '  '.implode($m);
        }$Ua = array_merge($Ua, array_filter($id));
        ...
        if (! queries('CREATE TABLE '.table($Ki)." (\n".implode(",\n", $Ua)."\n)")) {
            ...
            if ($pg && ! queries('INSERT INTO '.table($Ki).' ('.implode(', ', $pg).') SELECT '.implode(', ', array_map('Adminer\idf_escape', array_keys($pg))).' FROM '.table($R))) {
                return false;
            }$kj = [];
    ...
    }function drop_views($Oj)
    {
        return apply_queries('DROP VIEW', $Oj);
    }function drop_tables($T)
    {
        return apply_queries('DROP TABLE', $T);
    }function move_tables($T, $Oj, $Ii)
    {
        return false;
    }function trigger($B, $R)
    {
        ...
        return ['Timing' => strtoupper($A[1]), 'Event' => strtoupper($A[2]).($Mf ? ' OF' : ''), 'Of' => idf_unescape($Mf), 'Trigger' => $B, 'Statement' => $A[4]];
    }function triggers($R)
    {
        $J = [];
        $jj = trigger_options();
    ...
    }function use_sql($Nb, $si = '') {}function trigger_sql($R)
    {
    ...
    {
        $J = [];
        foreach (get_vals('PRAGMA compile_options') as $bg) {
            $J[] = explode('=', $bg, 2) + ['', ''];
        }

        return $J;
    }function convert_field($m) {}function unconvert_field($m, $J)
    {
        return $J;
    }function support($Tc)
    {
        return preg_match('~^(check|columns|database|drop_col|dump|indexes|descidx|move_col|sql|status|table|trigger|variables|view|view_trigger)$~', $Tc);
    }
...
if (isset($_GET['pgsql'])) {
    ...
    if (extension_loaded('pgsql') && $_GET['ext'] != 'pdo') {
        class PgsqlDb extends SqlDb
        {
            ...
            public function attach($N, $V, $F)
            {
                ...
                [$Ld, $Ng] = host_port(addcslashes($N, "'\\"));
                $this->string = "host='$Ld'".($Ng ? " port='$Ng'" : '')." user='".addcslashes($V, "'\\")."' password='".addcslashes($F, "'\\")."'";
            ...
            {
                $this->error = '';
                set_error_handler(function ($Bc, $l) {
        ...
        class PgsqlDb extends PdoDb
        {
            ...
            public function attach($N, $V, $F)
            {
                $j = adminer()->database();
                [$Ld, $Ng] = host_port(addcslashes($N, "'\\"));
                $mc = "pgsql:host='$Ld'".($Ng ? " port='$Ng'" : '')." client_encoding=utf8 dbname='".($j != '' ? addcslashes($j, "'\\") : 'postgres')."'";
                $ni = adminer()->connectSsl();
                if (isset($ni['mode'])) {
                    $mc
                    .= " sslmode='".$ni['mode']."'";
    ...
    }class Driver extends SqlDriver
    {
        ...
        public static function connect($N, $V, $F)
        {
            ...
            if (is_string($f)) {
                return $f;
            }$Mj = get_val('SELECT version()', 0, $f);
            $f->flavor = (preg_match('~CockroachDB~', $Mj) ? 'cockroach' : '');
            $f->server_info = preg_replace('~^\D*([\d.]+[-\w]*).*~', '\1', $Mj);
        ...
        public function enumLength(array $m)
        {
            $yc = $this->types['User types'][$m['type']];

            return $yc ? type_values($yc) : '';
        }

        public function setUserTypes($nj)
        {
        ...
        public function tableHelp($B, $xe = false)
        {
            $Re = ['information_schema' => 'infoschema', 'pg_catalog' => ($xe ? 'view' : 'catalog')];
            $_ = $Re[$_GET['ns']];
            if ($_) {
        ...
        public function partitionsInfo($R)
        {
            ...
            if ($K) {
                ...
                $Oa = ['h' => 'HASH', 'l' => 'LIST', 'r' => 'RANGE'];

                return ['partition_by' => $Oa[$K['partstrat']], 'partition' => implode(', ', array_map('Adminer\idf_escape', $ya))];
            }
...
ORDER BY datname");
    ...
    {
        return get_val('SELECT datcollate FROM pg_database WHERE datname = '.q($j));
    }function logged_user()
    {
...
ORDER BY 1';
    ...
    }function table_status($B = '')
    {
        static $Ed;
        if ($Ed === null) {
            $Ed = get_val("SELECT 'pg_table_size'::regproc");
        }$J = [];
...
'.($B != '' ? 'AND relname = '.q($B) : 'ORDER BY relname')) as $K) {
    ...
    {
        $J = [];
        $ra = ['timestamp without time zone' => 'timestamp', 'timestamp with time zone' => 'timestamptz'];
        foreach (get_rows('SELECT
	a.attname AS field,
...
ORDER BY indisprimary DESC, indisunique DESC", $g) as $K) {
            $rh = $K['relname'];
            $J[$rh]['type'] = ($K['indisprimary'] ? 'PRIMARY' : ($K['indisunique'] ? 'UNIQUE' : 'INDEX'));
            $J[$rh]['columns'] = [];
            $J[$rh]['descs'] = [];
            $J[$rh]['algorithm'] = $K['amname'];
            $J[$rh]['partial'] = $K['partial'];
            $de = preg_split('~(?<=\)), (?=\()~', $K['indexpr']);
            foreach (explode(' ', $K['indkey']) as $ee) {
                $J[$rh]['columns'][] = ($ee ? $e[$ee] : array_shift($de));
            }foreach (explode(' ', $K['indoption']) as $fe) {
                $J[$rh]['descs'][] = (intval($fe) & 1 ? '1' : null);
            }$J[$rh]['lengths'] = [];
        }
...
ORDER BY conkey, conname") as $K) {
    ...
    {
        return queries('CREATE DATABASE '.idf_escape($j).($c ? ' ENCODING '.idf_escape($c) : ''));
    }function drop_databases($i)
    {
        connection()->close();

        return apply_queries('DROP DATABASE', $i, 'Adminer\idf_escape');
    }function rename_database($B, $c)
    {
        connection()->close();
        ...
        foreach ($n as $m) {
            ...
            if (! $X) {
                ...
                if ($m[0] == '') {
                    ...
                    $eh[] = 'COMMENT ON COLUMN '.table($B).".$X[0] IS ".($Ij != '' ? substr($Ij, 9) : "''");
                }
            }
        }$b = array_merge($b, $id);
        if ($R == '') {
            ...
            if ($E) {
                ...
                if ($E['partition_by'] == 'HASH') {
                    $Dg = +$E['partitions'];
                    for ($s = 0; $s < $Dg; $s++) {
                        $eh[] = 'CREATE TABLE '.idf_escape($B."_$s").' PARTITION OF '.idf_escape($B)." FOR VALUES WITH (MODULUS $Dg, REMAINDER $s)";
                    }
                } else {
                    $Vg = 'MINVALUE';
                    foreach ($E['partition_names'] as $s => $X) {
                        $Y = $E['partition_values'][$s];
                        $_g = ' VALUES '.($E['partition_by'] == 'LIST' ? "IN ($Y)" : "FROM ($Vg) TO ($Y)");
                        if ($eb) {
                            $P
                            .= ($s ? ',' : ' (')."\n  PARTITION ".(preg_match('~^DEFAULT$~i', $X) ? $X : idf_escape($X))."$_g";
                        } else {
                            $eh[] = 'CREATE TABLE '.idf_escape($B."_$X").' PARTITION OF '.idf_escape($B)." FOR$_g";
                        }$Vg = $Y;
                    }$P
                    .= ($eb ? "\n)" : '');
                }
    ...
    }function alter_indexes($R, $b)
    {
        ...
        foreach ($b as $X) {
            ...
            array_unshift($eh, 'DROP INDEX '.implode(', ', $hc));
        }foreach ($eh as $H) {
            if (! queries($H)) {
                return false;
            }
...
ORDER BY event_manipulation DESC") as $K) {
    ...
    }function triggers($R)
    {
        ...
        foreach (get_rows('SELECT * FROM information_schema.triggers WHERE trigger_schema = current_schema() AND event_object_table = '.q($R)) as $K) {
            ...
            $J[$hj['Trigger']] = [$hj['Timing'], $hj['Event']];
        }
...
AND typelem = 0");
    ...
    {
        $Ac = get_vals("SELECT enumlabel FROM pg_enum WHERE enumtypid = $t ORDER BY enumsortorder");

        return $Ac ? "'".implode("', '", array_map('addslashes', $Ac))."'" : '';
    }function schemas()
    {
    ...
    }function create_sql($R, $_a, $si)
    {
        ...
        $Uh = [];
        $P = table_status1($R);
        $Hf = idf_escape($P['nspname']);
        if (is_view($P)) {
            $Nj = view($R);

            return rtrim("CREATE VIEW $Hf.".idf_escape($R)." AS $Nj[select]", ';');
        }$n = fields($R);
        if (count($P) < 2 || empty($n)) {
            return false;
        }$J = "CREATE TABLE $Hf.".idf_escape($P['Name'])." (\n    ";
        foreach ($n as $m) {
            $yg = idf_escape($m['field']).' '.$m['full_type'].default_value($m).($m['null'] ? '' : ' NOT NULL');
            $wh[] = $yg;
            if (preg_match('~nextval\(\'([^\']+)\'\)~', $m['default'], $Ze)) {
                ...
                $Uh[] = ($si == 'DROP+CREATE' ? "DROP SEQUENCE IF EXISTS $Hf.$Th;\n" : '')."CREATE SEQUENCE $Hf.$Th INCREMENT $hi[increment_by] MINVALUE $hi[min_value] MAXVALUE $hi[max_value]".($_a && $hi['last_value'] ? ' START '.($hi['last_value'] + 1) : '')." CACHE $hi[cache_value];";
            }
        }if (! empty($Uh)) {
            $J = implode("\n\n", $Uh)."\n\n$J";
        }$G = '';
        foreach (indexes($R) as $be => $v) {
            if ($v['type'] == 'PRIMARY') {
                $G = $be;
                $wh[] = 'CONSTRAINT '.idf_escape($be).' PRIMARY KEY ('.implode(', ', array_map('Adminer\idf_escape', $v['columns'])).')';
            }
        }foreach (driver()->checkConstraints($R) as $ub => $wb) {
            $wh[] = 'CONSTRAINT '.idf_escape($ub)." CHECK ($wb)";
        }$J
        .= implode(",\n    ", $wh)."\n)";
        $_g = driver()->partitionsInfo($P['Name']);
        if ($_g) {
            $J
            .= "\nPARTITION BY $_g[partition_by]($_g[partition])";
        }$J
        .= "\nWITH (oids = ".($P['Oid'] ? 'true' : 'false').');';
        if ($P['Comment']) {
            $J
            .= "\n\nCOMMENT ON TABLE $Hf.".idf_escape($P['Name']).' IS '.q($P['Comment']).';';
        }foreach ($n as $Vc => $m) {
            if ($m['comment']) {
                $J
                .= "\n\nCOMMENT ON COLUMN $Hf.".idf_escape($P['Name']).'.'.idf_escape($Vc).' IS '.q($m['comment']).';';
            }
    ...
    {
        return preg_match('~^(check|columns|comment|database|drop_col|dump|descidx|indexes|kill|partial_indexes|routine|scheme|sequence|sql|table|trigger|type|variables|view'.(min_version(9.3) ? '|materializedview' : '').(min_version(11) ? '|procedure' : '').(connection()->flavor == 'cockroach' ? '' : '|processlist').')$~', $Tc);
...
if (isset($_GET['oracle'])) {
    ...
    if (extension_loaded('oci8') && $_GET['ext'] != 'pdo') {
        class Db extends SqlDb
        {
            ...
            public function timeout($vf)
            {
                return oci_set_call_timeout($this->link, $vf);
            }
        }class Result
        {
            public $num_rows;
    ...
    }class Driver extends SqlDriver
    {
        ...
        {
            parent::__construct($f);
            $this->types = ['Numbers' => ['number' => 38, 'binary_float' => 12, 'binary_double' => 21], 'Date and time' => ['date' => 10, 'timestamp' => 29, 'interval year' => 12, 'interval day' => 28], 'Strings' => ['char' => 2000, 'varchar2' => 4000, 'nchar' => 2000, 'nvarchar2' => 4000, 'clob' => 4294967295, 'nclob' => 4294967295], 'Binary' => ['raw' => 2000, 'long raw' => 2147483648, 'blob' => 4294967295, 'bfile' => 4294967296]];
        }
...
ORDER BY 1');
    ...
    }function get_current_db()
    {
        ...
        return $j;
    }function where_owner($Tg, $sg = 'owner')
    {
        if (! $_GET['ns']) {
            return '';
        }

        return "$Tg$sg = sys_context('USERENV', 'CURRENT_SCHEMA')";
    }function views_table($e)
    {
        $sg = where_owner('');

        return "(SELECT $e FROM all_views WHERE ".($sg ?: 'rownum < 0').')';
    }function tables_list()
    {
        $Nj = views_table('view_name');
        $sg = where_owner(' AND ');
...
ORDER BY 1");
    ...
    }function table_status($B = '')
    {
        ...
        $Nj = views_table('view_name');
        $sg = where_owner(' AND ');
        foreach (get_rows('SELECT table_name "Name", \'table\' "Engine", avg_row_len * num_rows "Data_length", num_rows "Rows" FROM all_tables WHERE tablespace_name = '.q($j).$sg.($B != '' ? " AND table_name = $Kh" : '')."
...
ORDER BY 1') as $K) {
    ...
    }function fields($R)
    {
        $J = [];
        $sg = where_owner(' AND ');
        foreach (get_rows('SELECT * FROM all_tab_columns WHERE table_name = '.q($R)."$sg ORDER BY column_id") as $K) {
            $U = $K['DATA_TYPE'];
            $y = "$K[DATA_PRECISION],$K[DATA_SCALE]";
            if ($y == ',') {
                $y = $K['CHAR_COL_DECL_LENGTH'];
            }$J[$K['COLUMN_NAME']] = ['field' => $K['COLUMN_NAME'], 'full_type' => $U.($y ? "($y)" : ''), 'type' => strtolower($U), 'length' => $y, 'default' => $K['DATA_DEFAULT'], 'null' => ($K['NULLABLE'] == 'Y'), 'privileges' => ['insert' => 1, 'select' => 1, 'update' => 1, 'where' => 1, 'order' => 1]];
        }
    ...
    {
        $J = [];
        $sg = where_owner(' AND ', 'aic.table_owner');
...
ORDER BY ac.constraint_type, aic.column_position", $g) as $K) {
            ...
            $J[$be]['type'] = ($K['CONSTRAINT_TYPE'] == 'P' ? 'PRIMARY' : ($K['CONSTRAINT_TYPE'] == 'U' ? 'UNIQUE' : 'INDEX'));
            $J[$be]['columns'][] = $lb;
            $J[$be]['lengths'][] = ($K['CHAR_LENGTH'] && $K['CHAR_LENGTH'] != $K['COLUMN_LENGTH'] ? $K['CHAR_LENGTH'] : null);
            $J[$be]['descs'][] = ($K['DESCEND'] && $K['DESCEND'] == 'DESC' ? '1' : null);
        }

        return $J;
    }function view($B)
    {
        $Nj = views_table('view_name, text');
        $L = get_rows('SELECT text "select" FROM '.$Nj.' WHERE view_name = '.q($B));
    ...
    }function alter_table($R, $B, $n, $id, $ob, $wc, $c, $_a, $E)
    {
        ...
        return (! $b || queries('ALTER TABLE '.table($R)."\n".implode("\n", $b))) && (! $hc || queries('ALTER TABLE '.table($R).' DROP ('.implode(', ', $hc).')')) && ($R == $B || queries('ALTER TABLE '.table($R).' RENAME TO '.table($B)));
    }function alter_indexes($R, $b)
    {
        $hc = [];
        $eh = [];
...
if (isset($_GET['mssql'])) {
    ...
    if (extension_loaded('sqlsrv') && $_GET['ext'] != 'pdo') {
        class Db extends SqlDb
        {
            ...
            public function attach($N, $V, $F)
            {
                $vb = ['UID' => $V, 'PWD' => $F, 'CharacterSet' => 'UTF-8'];
                $ni = adminer()->connectSsl();
                if (isset($ni['Encrypt'])) {
                ...
                $this->link = @sqlsrv_connect($Ld.($Ng ? ",$Ng" : ''), $vb);
                if ($this->link) {
                    $ge = sqlsrv_server_info($this->link);
                    $this->server_info = $ge['SQLServerVersion'];
                } else {
                    $this->get_error();
                }
        ...
        }function last_id($I)
        {
        ...
        }function explain($f, $H) {}if (extension_loaded('pdo_sqlsrv')) {
            class Db extends MssqlDb
            {
                ...
                public function attach($N, $V, $F)
                {
                    ...
                    return $this->dsn("sqlsrv:Server=$Ld".($Ng ? ",$Ng" : ''), $V, $F);
                }
            }
            ...
            class Db extends MssqlDb
            {
                ...
                public function attach($N, $V, $F)
                {
                    ...
                    return $this->dsn("dblib:charset=utf8;host=$Ld".($Ng ? (is_numeric($Ng) ? ';port=' : ';unix_socket=').$Ng : ''), $V, $F);
                }
            }
        }
    } class Driver extends SqlDriver
    {
        ...
        {
            parent::__construct($f);
            $this->types = ['Numbers' => ['tinyint' => 3, 'smallint' => 5, 'int' => 10, 'bigint' => 20, 'bit' => 1, 'decimal' => 0, 'real' => 12, 'float' => 53, 'smallmoney' => 10, 'money' => 20], 'Date and time' => ['date' => 10, 'smalldatetime' => 19, 'datetime' => 19, 'datetime2' => 19, 'time' => 8, 'datetimeoffset' => 10], 'Strings' => ['char' => 8000, 'varchar' => 8000, 'text' => 2147483647, 'nchar' => 4000, 'nvarchar' => 4000, 'ntext' => 1073741823], 'Binary' => ['binary' => 8000, 'varbinary' => 8000, 'image' => 2147483647]];
        }
        ...
        public function tableHelp($B, $xe = false)
        {
            $Re = ['sys' => 'catalog-views/sys-', 'INFORMATION_SCHEMA' => 'information-schema-views/'];
            $_ = $Re[get_schema()];
            if ($_) {
    ...
    {
        return limit($H, $Z, 1, 0, $Rh);
    }function db_collation($j, $jb)
    {
        return get_val('SELECT collation_name FROM sys.databases WHERE name = '.q($j));
    }function logged_user()
    {
...
WHERE OBJECT_NAME(i.object_id) = '.q($R), $g) as $K) {
    ...
    {
        return queries('CREATE DATABASE '.idf_escape($j).(preg_match('~^[a-z0-9_]+$~i', $c) ? " COLLATE $c" : ''));
    }function drop_databases($i)
    {
        return queries('DROP DATABASE '.implode(', ', array_map('Adminer\idf_escape', $i)));
    }function rename_database($B, $c)
    {
        ...
        foreach ($n as $m) {
            ...
            if (! $X) {
                ...
                if (preg_match('~ AS ~', $X[3])) {
                    ...
                    $kg = $lg[$m[0]];
                    if (default_value($kg) != $k) {
                        if ($kg['default'] !== null) {
                            $b['DROP'][] = ' '.idf_escape($kg['default_constraint']);
                        }if ($k) {
                            $b['ADD'][] = "\n $k FOR $d";
                        }
                    }
...
@level2name = ".q($x));
    ...
    }function alter_indexes($R, $b)
    {
        ...
        return (! $v || queries('DROP INDEX '.implode(', ', $v))) && (! $hc || queries('ALTER TABLE '.table($R).' DROP '.implode(', ', $hc)));
    }function found_rows($S, $Z) {}function foreign_keys($R)
    {
        $J = [];
        $Vf = ['CASCADE', 'NO ACTION', 'SET NULL', 'SET DEFAULT'];
        foreach (get_rows('EXEC sp_fkeys @fktable_name = '.q($R).', @fktable_owner = '.q(get_schema())) as $K) {
            ...
            $p['on_update'] = $Vf[$K['UPDATE_RULE']];
            $p['on_delete'] = $Vf[$K['DELETE_RULE']];
    ...
    {
        return queries('DROP VIEW '.implode(', ', array_map('Adminer\table', $Oj)));
    }function drop_tables($T)
    {
        return queries('DROP TABLE '.implode(', ', array_map('Adminer\table', $T)));
    }function move_tables($T, $Oj, $Ii)
    {
        return apply_queries('ALTER SCHEMA '.idf_escape($Ii).' TRANSFER', array_merge($T, $Oj));
    }function trigger($B, $R)
    {
        if ($B == '') {
            return [];
...
WHERE sys1.xtype = 'TR' AND sys2.name = ".q($R)) as $K) {
    ...
    }function create_sql($R, $_a, $si)
    {
        if (is_view(table_status1($R))) {
            ...
            return 'CREATE VIEW '.table($R)." AS $Nj[select]";
        }$n = [];
        $G = false;
        foreach (fields($R) as $B => $m) {
            $X = process_field($m, $m);
    ...
    {
        $n = [];
        foreach (foreign_keys($R) as $id) {
            $n[] = ltrim(format_foreign_key($id));
        }
    ...
    {
        return preg_match('~^(check|comment|columns|database|drop_col|dump|indexes|descidx|scheme|sql|table|trigger|view|view_trigger)$~', $Tc);
    }
}class Adminer
{
    public static $instance;
    ...
    public function databases($gd = true)
    {
        return get_databases($gd);
    }
    ...
    public function csp(array $Gb)
    {
        return $Gb;
    }

    public function head($Kb = null)
    {
        return true;
    }
    ...
    public function css()
    {
        ...
        foreach (['', '-dark'] as $uf) {
            $o = "adminer$uf.css";
            if (file_exists($o)) {
                $Yc = file_get_contents($o);
                $J["$o?v=".crc32($Yc)] = ($uf ? 'dark' : (preg_match('~prefers-color-scheme:\s*dark~', $Yc) ? '' : 'light'));
            }
        }
    ...
    public function loginFormField($B, $Gd, $Y)
    {
        return $Gd.$Y."\n";
    }

    public function login($Te, $F)
    {
        if ($F == '') {
    ...
    public function tableName(array $yi)
    {
        return h($yi['Name']);
    }

    public function fieldName(array $m, $eg = 0)
    {
        $U = $m['full_type'];
        $ob = $m['comment'];
    ...
    public function selectLinks(array $yi, $O = '')
    {
        $B = $yi['Name'];
        echo '<p class="links">';
        $Re = ['select' => 'Select data'];
        ...
        }echo doc_link([JUSH => driver()->tableHelp($B, $xe)], '?'),"\n";
    }
    ...
    public function backwardKeys($R, $xi)
    {
        return [];
    }

    public function backwardKeysPrint(array $Da, array $K) {}

    public function selectQuery($H, $oi, $Rc = false)
    {
        $J = "</p>\n";
        if (! $Rc && ($Rj = driver()->warnings())) {
            $t = 'warnings';
    ...
    public function rowDescriptions(array $L, array $jd)
    {
        return $L;
    }
    ...
    public function selectVal($X, $_, array $m, $og)
    {
        ...
        if (is_blob($m) && ! is_utf8($X)) {
            $J = '<i>'.lang_format(['%d byte', '%d bytes'], strlen($og)).'</i>';
        }
    ...
    public function tableStructurePrint(array $n, $yi = null)
    {
    ...
    public function tableIndexesPrint(array $w, array $yi)
    {
        $zg = false;
        foreach ($w as $B => $v) {
            $zg |= (bool) $v['partial'];
        ...
        foreach ($w as $B => $v) {
            ...
            $Xg = [];
            foreach ($v['columns'] as $x => $X) {
                $Xg[] = '<i>'.h($X).'</i>'.($v['lengths'][$x] ? '('.$v['lengths'][$x].')' : '').($v['descs'][$x] ? ' DESC' : '');
            }echo "<tr title='".h($B)."'>","<th>$v[type]".($Sb && $v['algorithm'] != $Sb ? " ($v[algorithm])" : ''),'<td>'.implode(', ', $Xg);
            if ($zg) {
    ...
    public function selectColumnsPrint(array $M, array $e)
    {
        ...
        $M[''] = [];
        foreach ($M as $x => $X) {
            $X = idx($_GET['columns'], $x, []);
            $d = select_input(" name='columns[$s][col]'", $e, $X['col'], ($x !== '' ? 'selectFieldChange' : 'selectAddRow'));
            echo '<div>'.(driver()->functions || driver()->grouping ? html_select("columns[$s][fun]", [-1 => ''] + array_filter(['Functions' => driver()->functions, 'Aggregation' => driver()->grouping]), $X['fun']).on_help("event.target.value && event.target.value.replace(/ |\$/, '(') + ')'", 1).script("qsl('select').onchange = function () { helpClose();".($x !== '' ? '' : " qsl('select, input', this.parentNode).onchange();").' };', '')."($d)" : $d)."</div>\n";
            $s++;
        }echo "</div></fieldset>\n";
    }

    public function selectSearchPrint(array $Z, array $e, array $w)
    {
        print_fieldset('search', 'Search', $Z);
        foreach ($w as $s => $v) {
            if ($v['type'] == 'FULLTEXT') {
                echo '<div>(<i>'.implode('</i>, <i>', array_map('Adminer\h', $v['columns'])).'</i>) AGAINST'," <input type='search' name='fulltext[$s]' value='".h(idx($_GET['fulltext'], $s))."'>",script("qsl('input').oninput = selectFieldChange;", ''),(JUSH == 'sql' ? checkbox("boolean[$s]", 1, isset($_GET['boolean'][$s]), 'BOOL') : ''),"</div>\n";
            }
    ...
    public function selectOrderPrint(array $eg, array $e, array $w)
    {
        print_fieldset('sort', 'Sort', $eg);
        $s = 0;
    ...
    public function selectLengthPrint($Oi)
    {
        if ($Oi !== null) {
    ...
    public function selectEmailPrint(array $tc, array $e) {}

    public function selectColumnsProcess(array $e, array $w)
    {
        $M = [];
        $vd = [];
    ...
    public function selectSearchProcess(array $n, array $w)
    {
        $J = [];
        foreach ($w as $s => $v) {
            ...
            if ("$hb$X[val]" != '' && in_array($X['op'], adminer()->operators())) {
                $sb = [];
                foreach (($hb != '' ? [$hb => $n[$hb]] : $n) as $B => $m) {
                    $Tg = '';
                    $rb = " $X[op]";
                    if (preg_match('~IN$~', $X['op'])) {
                        $Vd = process_length($X['val']);
                        $rb
                        .= ' '.($Vd != '' ? $Vd : '(NULL)');
                    } elseif ($X['op'] == 'SQL') {
                        $rb = " $X[val]";
                        ...
                        $sb[] = $Tg.driver()->convertSearch(idf_escape($B), $X, $m).$rb;
                    }
    ...
    public function selectOrderProcess(array $n, array $w)
    {
        $J = [];
        foreach ((array) $_GET['order'] as $x => $X) {
            if ($X != '') {
    ...
    public function selectEmailProcess(array $Z, array $jd)
    {
        return false;
    }

    public function selectQueryBuild(array $M, array $Z, array $vd, array $eg, $z, $D)
    {
        return '';
    }

    public function messageQuery($H, $Pi, $Rc = false)
    {
        restart_session();
        ...
        }$Id[$_GET['db']][] = [$H, time(), $Pi];
        $ki = 'sql-'.count($Id[$_GET['db']]);
        $J = "<a href='#$ki' class='toggle'>".'SQL command'."</a> <a href='' class='jsonly copy'>ğŸ—</a>\n";
        if (! $Rc && ($Rj = driver()->warnings())) {
            $t = 'warnings-'.count($Id[$_GET['db']]);
            $J = "<a href='#$t' class='toggle'>".'Warnings'."</a>, $J<div id='$t' class='hidden'>\n$Rj</div>\n";
        }

        return " <span class='time'>".@date('H:i:s').'</span>'." $J<div id='$ki' class='hidden'><pre><code class='jush-".JUSH."'>".shorten_utf8($H, 1e4).'</code></pre>'.($Pi ? " <span class='time'>($Pi)</span>" : '').(support('sql') ? '<p><a href="'.h(str_replace('db='.urlencode(DB), 'db='.urlencode($_GET['db']), ME).'sql=&history='.(count($Id[$_GET['db']]) - 1)).'">'.'Edit'.'</a>' : '').'</div>';
    }

    public function editRowPrint($R, array $n, $K, $wj) {}

    public function editFunctions(array $m)
    {
        $J = ($m['null'] ? 'NULL/' : '');
        $wj = isset($_GET['select']) || where($_GET);
        foreach ([driver()->insertFunctions, driver()->editFunctions] as $x => $qd) {
            if (! $x || (! isset($_GET['call']) && $wj)) {
                foreach ($qd as $Hg => $X) {
                    if (! $Hg || preg_match("~$Hg~", $m['type'])) {
                        $J
                        .= "/$X";
                    }
                }
            }if ($x && $qd && ! preg_match('~set|bool~', $m['type']) && ! is_blob($m)) {
                $J
                .= '/SQL';
            }
    ...
    public function editInput($R, array $m, $ya, $Y)
    {
        if ($m['type'] == 'enum') {
    ...
    public function dumpTable($R, $si, $xe = 0)
    {
        if ($_POST['format'] != 'sql') {
            echo "\xef\xbb\xbf";
            if ($si) {
            ...
            if ($si && $h) {
                if ($si == 'DROP+CREATE' || $xe == 1) {
                    echo 'DROP '.($xe == 2 ? 'VIEW' : 'TABLE').' IF EXISTS '.table($R).";\n";
                }if ($xe == 1) {
                    $h = remove_definer($h);
                }echo "$h;\n\n";
            }
    ...
    public function dumpData($R, $si, $H)
    {
        if ($si) {
            ...
            if ($I) {
                ...
                while ($K = $I->$Uc()) {
                    if (! $Ce) {
                        ...
                        if (! $me) {
                            $me = 'INSERT INTO '.table($R).' ('.implode(', ', array_map('Adminer\idf_escape', $Ce)).') VALUES';
                        }foreach ($K as $x => $X) {
                            if ($rd[$x]) {
                                unset($K[$x]);
    ...
    public function dumpFilename($Pd)
    {
        return friendly_url($Pd != '' ? $Pd : (SERVER ?: 'localhost'));
    }

    public function dumpHeaders($Pd, $xf = false)
    {
        $rg = $_POST['output'];
        $Mc = (preg_match('~sql~', $_POST['format']) ? 'sql' : ($xf ? 'tar' : 'csv'));
        header('Content-Type: '.($rg == 'gz' ? 'application/x-gzip' : ($Mc == 'tar' ? 'application/x-tar' : ($Mc == 'sql' || $rg != 'file' ? 'text/plain' : 'text/csv').'; charset=utf-8')));
        if ($rg == 'gz') {
            ob_start(function ($Q) {
                return gzencode($Q);
            }, 1e6);
    ...
    public function navigation($tf)
    {
        echo '<h1>'.adminer()->name()." <span class='version'>".VERSION;
        $Ef = $_COOKIE['adminer_version'];
        echo " <a href='https://www.adminer.org/#download'".target_blank()." id='version'>".(version_compare(VERSION, $Ef) < 0 ? h($Ef) : '').'</a>',"</span></h1>\n";
        if ($tf == 'auth') {
            $rg = '';
            foreach ((array) $_SESSION['pwds'] as $Lj => $Wh) {
                foreach ($Wh as $N => $Fj) {
                    $B = h(get_setting("vendor-$Lj-$N") ?: get_driver($Lj));
                    foreach ($Fj as $V => $F) {
                        if ($F !== null) {
                            $Qb = $_SESSION['db'][$Lj][$N][$V];
                            foreach (($Qb ? array_keys($Qb) : ['']) as $j) {
                                $rg
            ...
            if ($Wd && function_exists('Adminer\alter_table')) {
        ...
        if (support('sql')) {
            ...
            if ($T) {
                ...
                foreach (['bac', 'bra', 'sqlite_quo', 'mssql_bra'] as $X) {
                    ...
                    $Ei = array_fill_keys(array_keys($T), []);
    ...
    public function databasesPrint($tf)
    {
        $i = adminer()->databases();
        ...
        echo "<label title='".'Database'."'>".'DB'.': '.($i ? html_select('db', ['' => ''] + $i, DB).$Ob : "<input name='db' value='".h(DB)."' autocapitalize='off' size='19'>\n").'</label>',"<input type='submit' value='".'Use'."'".($i ? " class='hidden'" : '').">\n";
        if (support('scheme')) {
            if ($tf != 'db' && DB != '' && connection()->select_db(DB)) {
                echo '<br><label>'.'Schema'.': '.html_select('ns', ['' => ''] + adminer()->schemas(), $_GET['ns'])."$Ob</label>";
                if ($_GET['ns'] != '') {
                    set_schema($_GET['ns']);
                }
            }
...
}class Plugins
{
    ...
    public function __construct($Mg)
    {
        if ($Mg === null) {
            ...
            if (file_exists("$Ha.php")) {
                ...
                if (is_array($Xd)) {
                    ...
                    $this->error
                    .= sprintf('%s must <a%s>return an array</a>.', "<b>$Ha.php</b>", $Hd).'<br>';
                }
            }foreach (get_declared_classes() as $db) {
                if (! $Mg[$db] && (preg_match('~^Adminer\w~i', $db) || is_subclass_of($db, 'Adminer\Plugin'))) {
                    $oh = new \ReflectionClass($db);
                    $xb = $oh->getConstructor();
                    if ($xb && $xb->getNumberOfRequiredParameters()) {
                        $this->error
                        .= sprintf('<a%s>Configure</a> %s in %s.', $Hd, "<b>$db</b>", "<b>$Ha.php</b>").'<br>';
                    } else {
                        $Mg[$db] = new $db;
                    }
                }
        ...
        $la = new Adminer;
        $Mg[] = $la;
        $oh = new \ReflectionObject($la);
        foreach ($oh->getMethods() as $rf) {
            foreach ($Mg as $Lg) {
                $B = $rf->getName();
                if (method_exists($Lg, $B)) {
                    $this->hooks[$B][] = $Lg;
                }
            }
        }
    ...
    public function includeOnce($o)
    {
        return include_once "./$o";
    }

    public function __call($B, array $wg)
    {
        $ua = [];
        foreach ($wg as $x => $X) {
            $ua[] = &$wg[$x];
        }$J = null;
        foreach ($this->hooks[$B] as $Lg) {
            $Y = call_user_func_array([$Lg, $B], $ua);
            if ($Y !== null) {
                if (! self::$append[$B]) {
                    return $Y;
                }$J = $Y + (array) $J;
...
}abstract class Plugin
{
    ...
    protected function lang($u, $Kf = null)
    {
        $ua = func_get_args();
...
if (! defined('Adminer\DRIVER')) {
    ...
    if (extension_loaded('mysqli') && $_GET['ext'] != 'pdo') {
        class Db extends \MySQLi
        {
            ...
            public function attach($N, $V, $F)
            {
                ...
                }$J = @$this->real_connect(($N != '' ? $Ld : ini_get('mysqli.default_host')), ($N.$V != '' ? $V : ini_get('mysqli.default_user')), ($N.$V.$F != '' ? $F : ini_get('mysqli.default_pw')), null, (is_numeric($Ng) ? intval($Ng) : ini_get('mysqli.default_port')), (is_numeric($Ng) ? null : $Ng), ($ni ? ($ni['verify'] !== false ? 2048 : 64) : 0));
                $this->options(MYSQLI_OPT_LOCAL_INFILE, 0);

                return $J ? '' : $this->error;
            }

            public function set_charset($Va)
            {
        ...
        class Db extends SqlDb
        {
            ...
            public function select_db($Nb)
            {
                return mysql_select_db($Nb, $this->link);
            }
        ...
        class Db extends PdoDb
        {
            ...
            public function attach($N, $V, $F)
            {
                $cg = [\PDO::MYSQL_ATTR_LOCAL_INFILE => false];
                $ni = adminer()->connectSsl();
                if ($ni) {
                    if ($ni['key']) {
                        $cg[\PDO::MYSQL_ATTR_SSL_KEY] = $ni['key'];
                    }if ($ni['cert']) {
                        $cg[\PDO::MYSQL_ATTR_SSL_CERT] = $ni['cert'];
                    }if ($ni['ca']) {
                        $cg[\PDO::MYSQL_ATTR_SSL_CA] = $ni['ca'];
                    }if (isset($ni['verify'])) {
                        $cg[\PDO::MYSQL_ATTR_SSL_VERIFY_SERVER_CERT] = $ni['verify'];
                    }
                }[$Ld, $Ng] = host_port($N);

                return $this->dsn("mysql:charset=utf8;host=$Ld".($Ng ? (is_numeric($Ng) ? ';port=' : ';unix_socket=').$Ng : ''), $V, $F, $cg);
            }
    ...
    } class Driver extends SqlDriver
    {
        ...
        public function insertUpdate($R, array $L, array $G)
        {
            ...
            $Tg = 'INSERT INTO '.table($R).' ('.implode(', ', $e).") VALUES\n";
            $Jj = [];
            foreach ($e as $x) {
                $Jj[$x] = "$x = VALUES($x)";
            }$ui = "\nON DUPLICATE KEY UPDATE ".implode(', ', $Jj);
            ...
            foreach ($L as $O) {
                $Y = '('.implode(', ', $O).')';
                if ($Jj && (strlen($Tg) + $y + strlen($Y) + strlen($ui) > 1e6)) {
                    if (! queries($Tg.implode(",\n", $Jj).$ui)) {
                        return false;
                    }$Jj = [];
                    $y = 0;
                }$Jj[] = $Y;
        ...
        public function tableHelp($B, $xe = false)
        {
            ...
            if (information_schema(DB)) {
                return strtolower('information-schema-'.($Ve ? "$B-table/" : str_replace('_', '-', $B).'-table.html'));
            }if (DB == 'mysql') {
        ...
        public function partitionsInfo($R)
        {
            ...
            $I = $this->conn->query("SELECT PARTITION_METHOD, PARTITION_EXPRESSION, PARTITION_ORDINAL_POSITION $od ORDER BY PARTITION_ORDINAL_POSITION DESC LIMIT 1");
            $J = [];
            [$J['partition_by'], $J['partition'], $J['partitions']] = $I->fetch_row();
            $Dg = get_key_vals("SELECT PARTITION_NAME, PARTITION_DESCRIPTION $od AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION");
            $J['partition_names'] = array_keys($Dg);
            $J['partition_values'] = array_values($Dg);
        ...
        public function indexAlgorithms(array $yi)
        {
            return preg_match('~^(MEMORY|NDB)$~', $yi['Engine']) ? ['HASH', 'BTREE'] : [];
        }
    }function idf_escape($u)
    {
        return '`'.str_replace('`', '``', $u).'`';
    ...
    {
        return limit($H, $Z, 1, 0, $Rh);
    }function db_collation($j, array $jb)
    {
        $J = null;
    ...
    {
        $J = [];
        foreach (get_rows($Sc ? 'SELECT TABLE_NAME AS Name, ENGINE AS Engine, TABLE_COMMENT AS Comment FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() '.($B != '' ? 'AND TABLE_NAME = '.q($B) : 'ORDER BY Name') : 'SHOW TABLE STATUS'.($B != '' ? ' LIKE '.q(addcslashes($B, '%_\\')) : '')) as $K) {
            if ($K['Engine'] == 'InnoDB') {
    ...
    }function fields($R)
    {
        ...
        foreach (get_rows('SELECT * FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = '.q($R).' ORDER BY ORDINAL_POSITION') as $K) {
            ...
            $Pc = $K['EXTRA'];
            preg_match('~^(VIRTUAL|PERSISTENT|STORED)~', $Pc, $rd);
            preg_match('~^([^( ]+)(?:\((.+)\))?( unsigned)?( zerofill)?$~', $U, $Ye);
            $k = $K['COLUMN_DEFAULT'];
            if ($k != '') {
                $we = preg_match('~text|json~', $Ye[1]);
                if (! $Ve && $we) {
            ...
            }$J[$m] = ['field' => $m, 'full_type' => $U, 'type' => $Ye[1], 'length' => $Ye[2], 'unsigned' => ltrim($Ye[3].$Ye[4]), 'default' => ($rd ? ($Ve ? $sd : stripslashes($sd)) : $k), 'null' => ($K['IS_NULLABLE'] == 'YES'), 'auto_increment' => ($Pc == 'auto_increment'), 'on_update' => (preg_match('~\bon update (\w+)~i', $Pc, $A) ? $A[1] : ''), 'collation' => $K['COLLATION_NAME'], 'privileges' => array_flip(explode(',', "$K[PRIVILEGES],where,order")), 'comment' => $K['COLUMN_COMMENT'], 'primary' => ($K['COLUMN_KEY'] == 'PRI'), 'generated' => ($rd[1] == 'PERSISTENT' ? 'STORED' : $rd[1])];
        }
    ...
    }function foreign_keys($R)
    {
        ...
        if ($Db) {
            preg_match_all("~CONSTRAINT ($Hg) FOREIGN KEY ?\\(((?:$Hg,? ?)+)\\) REFERENCES ($Hg)(?:\\.($Hg))? \\(((?:$Hg,? ?)+)\\)(?: ON DELETE (".driver()->onActions.'))?(?: ON UPDATE ('.driver()->onActions.'))?~', $Db, $Ze, PREG_SET_ORDER);
            foreach ($Ze as $A) {
                preg_match_all("~$Hg~", $A[2], $fi);
                preg_match_all("~$Hg~", $A[5], $Ii);
                $J[idf_unescape($A[1])] = ['db' => idf_unescape($A[4] != '' ? $A[3] : $A[4]), 'table' => idf_unescape($A[4] != '' ? $A[4] : $A[3]), 'source' => array_map('Adminer\idf_unescape', $fi[0]), 'target' => array_map('Adminer\idf_unescape', $Ii[0]), 'on_delete' => ($A[6] ?: 'RESTRICT'), 'on_update' => ($A[7] ?: 'RESTRICT')];
            }
        }
    ...
    {
        return queries('CREATE DATABASE '.idf_escape($j).($c ? ' COLLATE '.q($c) : ''));
    ...
    }function alter_table($R, $B, array $n, array $id, $ob, $wc, $c, $_a, $E)
    {
        $b = [];
        foreach ($n as $m) {
            if ($m[1]) {
        ...
        $P = ($ob !== null ? ' COMMENT='.q($ob) : '').($wc ? ' ENGINE='.q($wc) : '').($c ? ' COLLATE '.q($c) : '').($_a != '' ? " AUTO_INCREMENT=$_a" : '');
        if ($E) {
            $Dg = [];
    ...
    }function alter_indexes($R, $b)
    {
        ...
        return queries('ALTER TABLE '.table($R).implode(',', $Ua));
    }function truncate_tables(array $T)
    {
        return apply_queries('TRUNCATE TABLE', $T);
    }function drop_views(array $Oj)
    {
        return queries('DROP VIEW '.implode(', ', array_map('Adminer\table', $Oj)));
    }function drop_tables(array $T)
    {
        return queries('DROP TABLE '.implode(', ', array_map('Adminer\table', $T)));
    }function move_tables(array $T, array $Oj, $Ii)
    {
        $sh = [];
        foreach ($T as $R) {
            ...
            $j = idf_escape(DB);
            foreach ($Wb as $B => $Nj) {
                if (! queries("CREATE VIEW $B AS ".str_replace(" $j.", ' ', $Nj['select'])) || ! queries("DROP VIEW $j.$B")) {
                    return false;
                }
            }
        ...
        return false;
    }function copy_tables(array $T, array $Oj, $Ii)
    {
        ...
        foreach ($T as $R) {
            ...
            $B = ($Ii == DB ? table("copy_$R") : idf_escape($Ii).'.'.table($R));
            $Nj = view($R);
            if (($_POST['overwrite'] && ! queries("DROP VIEW IF EXISTS $B")) || ! queries("CREATE VIEW $B AS $Nj[select]")) {
                return false;
            }
        }
...
WHERE ROUTINE_SCHEMA = DATABASE() AND ROUTINE_TYPE = '$U' AND ROUTINE_NAME = ".q($B))->fetch_assoc();
    ...
    }function unconvert_field(array $m, $J)
    {
        if (preg_match('~binary~', $m['type'])) {
            $J = "UNHEX($J)";
        }if ($m['type'] == 'bit') {
            ...
            $Tg = (min_version(8) ? 'ST_' : '');
            $J = $Tg."GeomFromText($J, $Tg"."SRID($m[field]))";
        }

        return $J;
    }function support($Tc)
    {
        return preg_match('~^(comment|columns|copy|database|drop_col|dump|indexes|kill|privileges|move_col|procedure|processlist|routine|sql|status|table|trigger|variables|view'.(min_version(5.1) ? '|event' : '').(min_version(8) ? '|descidx' : '').(min_version('8.0.16', '10.2.1') ? '|check' : '').')$~', $Tc);
...
function page_header($Si, $l = '', $Ma = [], $Ti = '')
{
    page_headers();
    if (is_ajax() && $l) {
        page_messages($l);
...
';
    ...
    }$Dd = in_array('light', $Hb) || in_array('', $Hb);
    $Bd = in_array('dark', $Hb) || in_array('', $Hb);
    $Kb = ($Dd ? ($Bd ? null : false) : ($Bd ?: null));
    $jf = " media='(prefers-color-scheme: dark)'";
    if ($Kb !== false) {
        echo "<link rel='stylesheet'".($Kb ? '' : $jf)." href='".h(preg_replace('~\\?.*~', '', ME).'?file=dark.css&version=5.4.2')."'>\n";
    ...
    if (adminer()->head($Kb)) {
        ...
        $ya = ($uf == 'dark' && ! $Kb ? $jf : ($uf == 'light' && $Bd ? " media='(prefers-color-scheme: light)'" : ''));
        ...
        if ($Ma === false) {
            ...
            if ($_GET['ns'] != '' || (DB != '' && is_array($Ma))) {
                ...
                if ($_GET['ns'] != '') {
                    ...
                    if ($Yb != '') {
                        echo "<a href='".h(ME."$x=").urlencode(is_array($X) ? $X[0] : $X)."'>$Yb</a> Â» ";
                    }
                }
            }echo "$Si\n";
        }
...
}function page_messages($l)
{
    ...
    $qf = idx($_SESSION['messages'], $zj);
    if ($qf) {
        echo "<div class='message'>".implode("</div>\n<div class='message'>", $qf).'</div>'.script('messagesPrint();');
...
}function page_footer($tf = '')
{
    echo "</div>\n\n<div id='foot' class='foot'>\n<div id='menu'>\n";
    adminer()->navigation($tf);
    echo "</div>\n";
    if ($tf != 'auth') {
...
}function long2str(array $W, $Qj)
{
    ...
    foreach ($W as $X) {
        ...
        return substr($Dh, 0, end($W));
    }

    return $Dh;
}function str2long($Dh, $Qj)
{
    ...
    return $W;
}function xxtea_mx($Xj, $Wj, $vi, $Ae)
{
    return int32((($Xj >> 5 & 0x7FFFFFF) ^ $Wj << 2) + (($Wj >> 3 & 0x1FFFFFFF) ^ $Xj << 4)) ^ int32(($vi ^ $Wj) + ($Ae ^ $Xj));
}function encrypt_string($qi, $x)
{
    if ($qi == '') {
        return '';
    ...
    while ($dh-- > 0) {
        ...
        for ($tg = 0; $tg < $zf; $tg++) {
            $Wj = $W[$tg + 1];
            $yf = xxtea_mx($Xj, $Wj, $vi, $x[$tg & 3 ^ $nc]);
            $Xj = int32($W[$tg] + $yf);
            $W[$tg] = $Xj;
        }$Wj = $W[0];
        $yf = xxtea_mx($Xj, $Wj, $vi, $x[$tg & 3 ^ $nc]);
...
}function add_invalid_login()
{
    ...
    foreach (glob("$Fa*") ?: [$Fa] as $o) {
        ...
        return;
    }$re = unserialize(stream_get_contents($q));
    $Pi = time();
    if ($re) {
        foreach ($re as $se => $X) {
            if ($X[0] < $Pi) {
    ...
    }$qe = &$re[adminer()->bruteForceKey()];
    if (! $qe) {
        $qe = [$Pi + 30 * 60, 0];
    }$qe[1]++;
    file_write_unlock($q, serialize($re));
}function check_invalid_login(array &$Jg)
{
    $re = [];
    ...
    }$qe = idx($re, adminer()->bruteForceKey(), []);
    $Ff = ($qe[1] > 29 ? $qe[0] - time() : 0);
    if ($Ff > 0) {
        auth_error(lang_format(['Too many unsuccessful logins, try again in %d minute.', 'Too many unsuccessful logins, try again in %d minutes.'], ceil($Ff / 60)), $Jg);
    }
}$za = $_POST['auth'];
if ($za) {
    ...
    $j = $za['db'];
    set_password($Lj, $N, $V, $F);
    $_SESSION['db'][$Lj][$N][$V][$j] = true;
    if ($za['permanent']) {
        $x = implode('-', array_map('base64_encode', [$Lj, $N, $V, $j]));
        $Yg = adminer()->permanentLogin(true);
        $Jg[$x] = "$x:".base64_encode($Yg ? encrypt_string($F, $Yg) : '');
        cookie('adminer_permanent', implode(' ', $Jg));
    }if (count($_POST) == 1 || $Lj != DRIVER || $N != SERVER || $_GET['username'] !== $V || $j != DB) {
        redirect(auth_url($Lj, $N, $V, $j));
    }
} elseif ($_POST['logout'] && (! $_SESSION['token'] || verify_token())) {
    foreach (['pwds', 'db', 'dbs', 'queries'] as $x) {
        set_session($x, null);
    }unset_permanent($Jg);
...
} elseif ($Jg && ! $_SESSION['pwds']) {
    session_regenerate_id();
    $Yg = adminer()->permanentLogin();
    foreach ($Jg as $x => $X) {
        [, $cb] = explode(':', $X);
        [$Lj, $N, $V, $j] = array_map('base64_decode', explode('-', $x));
        set_password($Lj, $N, $V, decrypt_string(base64_decode($cb), $Yg));
        $_SESSION['db'][$Lj][$N][$V][$j] = true;
    }
}function unset_permanent(array &$Jg)
{
    foreach ($Jg as $x => $X) {
        [$Lj, $N, $V, $j] = array_map('base64_decode', explode('-', $x));
        if ($Lj == DRIVER && $N == SERVER && $V == $_GET['username'] && $j == DB) {
            unset($Jg[$x]);
        }
    }cookie('adminer_permanent', implode(' ', $Jg));
}function auth_error($l, array &$Jg)
{
    $Xh = session_name();
    ...
    }$wg = session_get_cookie_params();
    cookie('adminer_key', ($_COOKIE['adminer_key'] ?: rand_string()), $wg['lifetime']);
    if (! $_SESSION['token']) {
...
if (isset($_GET['username']) && is_string(get_password())) {
    [, $Ng] = host_port(SERVER);
    if (preg_match('~^\s*([-+]?\d+)~', $Ng, $A) && ($A[1] < 1024 || $A[1] > 65535)) {
        auth_error('Connecting to privileged ports is not allowed.', $Jg);
    }check_invalid_login($Jg);
    $Fb = adminer()->credentials();
...
if ($_POST) {
    if (! verify_token()) {
        ...
        if (extension_loaded('suhosin')) {
            foreach (['suhosin.request.max_vars', 'suhosin.post.max_vars'] as $x) {
                $X = ini_get($x);
                if ($X && (! $hf || $X < $hf)) {
                    $je = $x;
                    $hf = $X;
        ...
        }$l = (! $_POST['token'] && $hf ? sprintf('Maximum number of allowed fields exceeded. Please increase %s.', "'$je'") : 'Invalid CSRF token. Send the form again.'.' '.'If you did not send this request from Adminer then close this page.');
    }
...
}function print_select_result($I, $g = null, array $ig = [], $z = 0)
{
    $Re = [];
    $w = [];
    $e = [];
    ...
    for ($s = 0; (! $z || $s < $z) && ($K = $I->fetch_row()); $s++) {
        if (! $s) {
            echo "<div class='scrollable'>\n","<table class='nowrap odds'>\n",'<thead><tr>';
            for ($ye = 0; $ye < count($K); $ye++) {
                $m = $I->fetch_field();
                $B = $m->name;
                $hg = (isset($m->orgtable) ? $m->orgtable : '');
                $gg = (isset($m->orgname) ? $m->orgname : $B);
                if ($ig && JUSH == 'sql') {
                    $Re[$ye] = ($B == 'table' ? 'table=' : ($B == 'possible_keys' ? 'indexes=' : null));
                } elseif ($hg != '') {
                    if (isset($m->table)) {
                        $J[$m->table] = $hg;
                    }if (! isset($w[$hg])) {
                        $w[$hg] = [];
                        foreach (indexes($hg, $g) as $v) {
                            if ($v['type'] == 'PRIMARY') {
                                $w[$hg] = array_flip($v['columns']);
                                break;
                            }
                        }$e[$hg] = $w[$hg];
                    }if (isset($e[$hg][$gg])) {
                ...
                echo '<th'.($hg != '' || $m->name != $gg ? " title='".h(($hg != '' ? "$hg." : '').$gg)."'" : '').'>'.h($B).($ig ? doc_link(['sql' => 'explain-output.html#explain_'.strtolower($B), 'mariadb' => 'explain/#the-columns-in-explain-select']) : '');
            }echo "</thead>\n";
        }echo '<tr>';
        foreach ($K as $x => $X) {
            $_ = '';
            if (isset($Re[$x]) && ! $e[$Re[$x]]) {
                if ($ig && JUSH == 'sql') {
                    ...
                    $_ = ME.$Re[$x].urlencode($ig[$R] != '' ? $ig[$R] : $R);
                } else {
                    $_ = ME.'edit='.urlencode($Re[$x]);
                    foreach ($w[$Re[$x]] as $hb => $ye) {
                        if ($K[$ye] === null) {
                            $_ = '';
                            break;
                        }$_
                        .= '&where'.urlencode('['.bracket_escape($hb).']').'='.urlencode($K[$ye]);
                    }
                }
            }$m = ['type' => ($Ka[$x] ? 'blob' : ($nj[$x] == 254 ? 'char' : ''))];
            $X = select_value($X, $_, $m, null);
            echo '<td'.($nj[$x] <= 9 || $nj[$x] == 246 ? " class='number'" : '').">$X";
        }
    }echo ($s ? "</table>\n</div>" : "<p class='message'>".'No rows.')."\n";

    return $J;
}function referencable_primary($Ph)
{
    $J = [];
    ...
    return $J;
}function textarea($B, $Y, $L = 10, $kb = 80)
{
...
}function select_input($ya, array $cg, $Y = '', $Wf = '', $Kg = '')
{
    $Hi = ($cg ? 'select' : 'input');

    return "<$Hi$ya".($cg ? "><option value=''>$Kg".optionlist($cg, $Y, true).'</select>' : " size='10' value='".h($Y)."' placeholder='$Kg'>").($Wf ? script("qsl('$Hi').onchange = $Wf;", '') : '');
}function json_row($x, $X = null, $Ec = true)
{
    static $ad = true;
    if ($ad) {
        echo '{';
...
}function edit_type($x, array $m, array $jb, array $kd = [], array $Qc = [])
{
    $U = $m['type'];
    echo "<td><select name='".h($x)."[type]' class='type' aria-labelledby='label-type'>";
    if ($U && ! array_key_exists($U, driver()->types()) && ! isset($kd[$U]) && ! in_array($U, $Qc)) {
        $Qc[] = $U;
    }$ri = driver()->structuredTypes();
    if ($kd) {
        $ri['Foreign keys'] = $kd;
    }echo optionlist(array_merge($Qc, $ri), $U),'</select><td>',"<input name='".h($x)."[length]' value='".h($m['length'])."' size='3'".(! $m['length'] && preg_match('~var(char|binary)$~', $U) ? " class='required'" : '')." aria-labelledby='label-length'>","<td class='options'>",($jb ? "<input list='collations' name='".h($x)."[collation]'".(preg_match('~(char|text|enum|set)$~', $U) ? '' : " class='hidden'")." value='".h($m['collation'])."' placeholder='(".'collation'.")'>" : ''),(driver()->unsigned ? "<select name='".h($x)."[unsigned]'".(! $U || preg_match(number_type(), $U) ? '' : " class='hidden'").'><option>'.optionlist(driver()->unsigned, $m['unsigned']).'</select>' : ''),(isset($m['on_update']) ? "<select name='".h($x)."[on_update]'".(preg_match('~timestamp|datetime~', $U) ? '' : " class='hidden'").'>'.optionlist(['' => '('.'ON UPDATE'.')', 'CURRENT_TIMESTAMP'], (preg_match('~^CURRENT_TIMESTAMP~i', $m['on_update']) ? 'CURRENT_TIMESTAMP' : $m['on_update'])).'</select>' : ''),($kd ? "<select name='".h($x)."[on_delete]'".(preg_match('~`~', $U) ? '' : " class='hidden'")."><option value=''>(".'ON DELETE'.')'.optionlist(explode('|', driver()->onActions), $m['on_delete']).'</select> ' : ' ');
}function process_length($y)
{
    $_c = driver()->enumLength;

    return preg_match("~^\\s*\\(?\\s*$_c(?:\\s*,\\s*$_c)*+\\s*\\)?\\s*\$~", $y) && preg_match_all("~$_c~", $y, $Ze) ? '('.implode(',', $Ze[0]).')' : preg_replace('~^[0-9].*~', '(\0)', preg_replace('~[^-0-9,+()[\]]~', '', $y));
}function process_type(array $m, $ib = 'COLLATE')
{
...
}function process_field(array $m, array $mj)
{
    if ($m['on_update']) {
        $m['on_update'] = str_ireplace('current_timestamp()', 'CURRENT_TIMESTAMP', $m['on_update']);
    }

    return [idf_escape(trim($m['field'])), process_type($mj), ($m['null'] ? ' NULL' : ' NOT NULL'), default_value($m), (preg_match('~timestamp|datetime~', $m['type']) && $m['on_update'] ? " ON UPDATE $m[on_update]" : ''), (support('comment') && $m['comment'] != '' ? ' COMMENT '.q($m['comment']) : ''), ($m['auto_increment'] ? auto_increment() : null)];
}function default_value(array $m)
{
    if ($m['default'] === null) {
        return '';
    }$k = str_replace("\r", '', $m['default']);
    $rd = $m['generated'];

    return in_array($rd, driver()->generated) ? (JUSH == 'mssql' ? " AS ($k)".($rd == 'VIRTUAL' ? '' : " $rd") : " GENERATED ALWAYS AS ($k) $rd") : ' DEFAULT '.(! preg_match('~^GENERATED ~i', $k) && (preg_match('~char|binary|text|json|enum|set~', $m['type']) || preg_match('~^(?![a-z])~i', $k)) ? (JUSH == 'sql' && preg_match('~text|json~', $m['type']) ? '('.q($k).')' : q($k)) : str_ireplace('current_timestamp()', 'CURRENT_TIMESTAMP', (JUSH == 'sqlite' ? "($k)" : $k)));
}function type_class($U)
{
    foreach (['char' => 'text', 'date' => 'time|year', 'binary' => 'blob', 'enum' => 'set'] as $x => $X) {
        if (preg_match("~$x|$X~", $U)) {
            return " class='$x'";
        }
    }
}function edit_fields(array $n, array $jb, $U = 'TABLE', array $kd = [])
{
    $n = array_values($n);
    ...
    if ($U == 'TABLE') {
        echo "<td id='label-null'>NULL\n","<td><input type='radio' name='auto_increment_col' value=''><abbr id='label-ai' title='".'Auto Increment'."'>AI</abbr>",doc_link(['sql' => 'example-auto-increment.html', 'mariadb' => 'auto_increment/', 'sqlite' => 'autoinc.html', 'pgsql' => 'datatype-numeric.html#DATATYPE-SERIAL', 'mssql' => 't-sql/statements/create-table-transact-sql-identity-property']),"<td id='label-default'$Tb>".'Default value',(support('comment') ? "<td id='label-comment'$pb>".'Comment' : '');
    ...
    foreach ($n as $s => $m) {
        $s++;
        $jg = $m[($_POST ? 'orig' : 'field')];
        $dc = (isset($_POST['add'][$s - 1]) || (isset($m['field']) && ! idx($_POST['drop_col'], $s))) && (support('drop_col') || $jg == '');
        ...
        }echo input_hidden("fields[$s][orig]", $jg);
        edit_type("fields[$s]", $m, $jb, $kd);
        if ($U == 'TABLE') {
            echo '<td>'.checkbox("fields[$s][null]", 1, $m['null'], '', '', 'block', 'label-null'),"<td><label class='block'><input type='radio' name='auto_increment_col' value='$s'".($m['auto_increment'] ? ' checked' : '')." aria-labelledby='label-ai'></label>","<td$Tb>".(driver()->generated ? html_select("fields[$s][generated]", array_merge(['', 'DEFAULT'], driver()->generated), $m['generated']).' ' : checkbox("fields[$s][generated]", 1, $m['generated'], '', '', '', 'label-default'));
            $ya = " name='fields[$s][default]' aria-labelledby='label-default'";
            $Y = h($m['default']);
            echo (preg_match('~\n~', $m['default']) ? "<textarea$ya rows='2' cols='30' style='vertical-align: bottom;'>\n$Y</textarea>" : "<input$ya value='$Y'>"),(support('comment') ? "<td$pb><input name='fields[$s][comment]' value='".h($m['comment'])."' data-maxlength='".(min_version(5.5) ? 1024 : 255)."' aria-labelledby='label-comment'>" : '');
        }echo '<td>',(support('move_col') ? icon('plus', "add[$s]", '+', 'Add next').' '.icon('up', "up[$s]", 'â†‘', 'Move up').' '.icon('down', "down[$s]", 'â†“', 'Move down').' ' : ''),($jg == '' || support('drop_col') ? icon('cross', "drop_col[$s]", 'x', 'Remove') : '');
    }
...
}function grant($td, array $ah, $e, $Tf)
{
    if (! $ah) {
        return true;
    }if ($ah == ['ALL PRIVILEGES', 'GRANT OPTION']) {
        return $td == 'GRANT' ? queries("$td ALL PRIVILEGES$Tf WITH GRANT OPTION") : queries("$td ALL PRIVILEGES$Tf") && queries("$td GRANT OPTION$Tf");
    }

    return queries("$td ".preg_replace('~(GRANT OPTION)\([^)]*\)~', '\1', implode("$e, ", $ah).$e).$Tf);
}function drop_create($hc, $h, $jc, $Li, $lc, $Se, $pf, $nf, $of, $Qf, $Cf)
{
    if ($_POST['drop']) {
        query_redirect($hc, $Se, $pf);
    } elseif ($Qf == '') {
        query_redirect($h, $Se, $of);
    } elseif ($Qf != $Cf) {
        $Eb = queries($h);
        queries_redirect($Se, $nf, $Eb && queries($hc));
        if ($Eb) {
            queries($jc);
        }
    } else {
        queries_redirect($Se, $nf, queries($Li) && queries($lc) && queries($hc) && queries($h));
    }
}function create_trigger($Tf, array $K)
{
    $Ri = " $K[Timing] $K[Event]".(preg_match('~ OF~', $K['Event']) ? " $K[Of]" : '');

    return 'CREATE TRIGGER '.idf_escape($K['Trigger']).(JUSH == 'mssql' ? $Tf.$Ri : $Ri.$Tf).rtrim(" $K[Type]\n$K[Statement]", ';').';';
}function create_routine($_h, array $K)
{
    $O = [];
    $n = (array) $K['fields'];
    ksort($n);
...
}function format_foreign_key(array $p)
{
    ...
    return ' FOREIGN KEY ('.implode(', ', array_map('Adminer\idf_escape', $p['source'])).') REFERENCES '.($j != '' && $j != $_GET['db'] ? idf_escape($j).'.' : '').($Hf != '' && $Hf != $_GET['ns'] ? idf_escape($Hf).'.' : '').idf_escape($p['table']).' ('.implode(', ', array_map('Adminer\idf_escape', $p['target'])).')'.(preg_match('~^('.driver()->onActions.')$~', $p['on_delete']) ? " ON DELETE $p[on_delete]" : '').(preg_match('~^('.driver()->onActions.')$~', $p['on_update']) ? " ON UPDATE $p[on_update]" : '').($p['deferrable'] ? " $p[deferrable]" : '');
}function tar_file($o, $Wi)
{
    ...
    $Wi->send();
    echo str_repeat("\0", 511 - ($Wi->size + 511) % 512);
}function doc_link(array $Gg, $Mi = '<sup>?</sup>')
{
    $Vh = connection()->server_info;
    $Mj = preg_replace('~^(\d\.?\d).*~s', '\1', $Vh);
    $Aj = ['sql' => "https://dev.mysql.com/doc/refman/$Mj/en/", 'sqlite' => 'https://www.sqlite.org/', 'pgsql' => 'https://www.postgresql.org/docs/'.(connection()->flavor == 'cockroach' ? 'current' : $Mj).'/', 'mssql' => 'https://learn.microsoft.com/en-us/sql/', 'oracle' => 'https://www.oracle.com/pls/topic/lookup?ctx=db'.preg_replace('~^.* (\d+)\.(\d+)\.\d+\.\d+\.\d+.*~s', '\1\2', $Vh).'&id='];
    if (connection()->flavor == 'maria') {
        $Aj['sql'] = 'https://mariadb.com/kb/en/';
        $Gg['sql'] = (isset($Gg['mariadb']) ? $Gg['mariadb'] : str_replace('.html', '/', $Gg['sql']));
    }

    return $Gg[JUSH] ? "<a href='".h($Aj[JUSH].$Gg[JUSH].(JUSH == 'mssql' ? "?view=sql-server-ver$Mj" : ''))."'".target_blank().">$Mi</a>" : '';
}function db_size($j)
{
    if (! connection()->select_db($j)) {
        return '?';
...
}function set_utf8mb4($h)
{
    ...
    if (DB != '' || $_GET['refresh']) {
        ...
        echo "<p class='links'>\n";
        foreach (['database' => 'Create database', 'privileges' => 'Privileges', 'processlist' => 'Process list', 'variables' => 'Variables', 'status' => 'Status'] as $x => $X) {
            if (support($x)) {
        ...
        if ($i) {
            ...
            foreach (adminer()->plugins as $Lg) {
                $Zb = (method_exists($Lg, 'description') ? $Lg->description() : '');
                if (! $Zb) {
                    $oh = new \ReflectionObject($Lg);
                    if (preg_match('~^/[\s*]+(.+)~', $oh->getDocComment(), $A)) {
                        $Zb = $A[1];
                    }
                }$Ih = (method_exists($Lg, 'screenshot') ? $Lg->screenshot() : '');
                echo '<li><b>'.get_class($Lg).'</b>'.h($Zb ? ": $Zb" : '').($Ih ? " (<a href='".h($Ih)."'".target_blank().'>'.'screenshot'.'</a>)' : '')."\n";
            }echo "</ul>\n";
...
class TmpFile
{
    ...
    $I = driver()->select($a, $M, [where($_GET, $n)], $M);
    $K = ($I ? $I->fetch_row() : []);
    echo driver()->value($K[0], $n[$_GET['field']]);
    exit;
    ...
    }$he = driver()->inheritedTables($a);
    if ($he) {
        echo "<h3 id='partitions'>".'Inherited by'."</h3>\n";
        $_g = driver()->partitionsInfo($a);
        if ($_g) {
            echo "<p><code class='jush-".JUSH."'>BY ".h("$_g[partition_by]($_g[partition])")."</code>\n";
        }tables_links($he);
    }
        ...
        foreach (adminer()->foreignKeys($R) as $X) {
            if (! $X['db']) {
                ...
                if (idx($Bi[$R], 1) || idx($Bi[$X['table']], 1)) {
                    ...
                    $Ke -= .0001;
                }$Fh[$R]['references'][$X['table']][(string) $Ke] = [$X['source'], $X['target']];
                $nh[$X['table']][$R][(string) $Ke] = $X['target'];
                $Me[(string) $Ke] = true;
            }
        }$Zi = max($Zi, $Fh[$R]['pos'][0] + 2.5 + $Og);
        ...
        foreach ((array) $R['references'] as $Ji => $ph) {
            foreach ($ph as $Ke => $kh) {
                ...
                foreach ($kh[0] as $x => $fi) {
                    $Pg = $R['pos'][0] + $R['fields'][$fi]['pos'];
                    $Qg = $Fh[$Ji]['pos'][0] + $Fh[$Ji]['fields'][$kh[1][$x]]['pos'];
                    $sf = min($sf, $Pg, $Qg);
                    $ff = max($ff, $Pg, $Qg);
                }echo "<div class='references' id='refl$Ke' style='left: $Ke"."em; top: $sf"."em; padding: .5em 0;'><div style='border-right: 1px solid gray; margin-top: 1px; height: ".($ff - $sf)."em;'></div></div>\n";
            }
        }
    }echo '</div>
...
';
    ...
    if ($_POST && ! $l) {
        ...
        $Mc = dump_headers((count($T) == 1 ? key($T) : DB), (DB == '' || count($T) > 1));
        $ve = preg_match('~sql~', $_POST['format']);
        if ($ve) {
...
';
                ...
                if ($ve) {
                    ...
                    if ($_POST['types']) {
                        ...
                        foreach (routines() as $K) {
                            $B = $K['ROUTINE_NAME'];
                            $_h = $K['ROUTINE_TYPE'];
                            $h = create_routine($_h, ['name' => $B] + routine($K['SPECIFIC_NAME'], $_h));
                            set_utf8mb4($h);
                            $qg
                    ...
                    foreach (table_status('', true) as $B => $S) {
                        ...
                        if ($R || $Lb) {
                            ...
                            }if ($ve && $_POST['triggers'] && $R && ($kj = trigger_sql($B))) {
                                echo "\nDELIMITER ;;\n$kj\nDELIMITER ;\n";
                            }if ($Mc == 'tar') {
                                ob_end_flush();
                        ...
                        adminer()->dumpTable($Nj, $_POST['table_style'], 1);
                    }if ($Mc == 'tar') {
                        echo pack('x512');
                    }
                }
        ...
        exit;
    }page_header('Export', $l, ($_GET['export'] != '' ? ['table' => $_GET['export']] : []), h(DB));
    echo '
<form action="" method="post">
<table class="layout">
';
    $Pb = ['', 'USE', 'DROP+CREATE', 'CREATE'];
    $Di = ['', 'DROP+CREATE', 'CREATE'];
    $Mb = ['', 'TRUNCATE+INSERT', 'INSERT'];
    if (JUSH == 'sql') {
        $Mb[] = 'INSERT+UPDATE';
...
',script("qsl('table').onclick = dumpClick;");
    ...
    if (DB != '') {
        ...
        foreach ($Fi as $B => $U) {
            $Tg = preg_replace('~_.*~', '', $B);
            $Za = ($a == '' || $a == (substr($a, -1) == '%' ? "$Tg%" : $B));
            $Xg = '<tr><td>'.checkbox('tables[]', $B, $Za, $B, '', 'block');
        ...
        if ($i) {
            foreach ($i as $j) {
                if (! information_schema($j)) {
                    $Tg = preg_replace('~_.*~', '', $j);
                    echo '<tr><td>'.checkbox('databases[]', $j, $a == '' || $a == "$Tg%", $j, '', 'block')."\n";
                    $Ug[$Tg]++;
                }
            }
        } else {
...
';
    ...
    foreach ($Ug as $x => $X) {
        if ($x != '' && $X > 1) {
            echo ($ad ? '<p>' : ' ')."<a href='".h(ME).'dump='.urlencode("$x%")."'>".h($x).'</a>';
            $ad = false;
        }
    }
    ...
    hidden_fields_get();
    echo input_hidden('db', DB),($td ? '' : input_hidden('grant')),"<table class='odds'>\n",'<thead><tr><th>'.'Username'.'<th>'.'Server'."<th></thead>\n";
    while ($K = $I->fetch_assoc()) {
    ...
    if (! $l && $_POST) {
        ...
        if (! isset($_GET['import'])) {
            ...
            $Cc = [];
            $xg = '[\'"'.(JUSH == 'sql' ? '`#' : (JUSH == 'sqlite' ? '`[' : (JUSH == 'mssql' ? '[' : ''))).']|/\*|'.$Qe.'|$'.(JUSH == 'pgsql' ? '|\$([a-zA-Z]\w*)?\$' : '');
            $aj = microtime(true);
            ...
            while ($H != '') {
                if (! $C && preg_match("~^$gi*+DELIMITER\\s+(\\S+)~i", $H, $A)) {
                    ...
                    preg_match("($Xb\\s*|$xg)", $H, $A, PREG_OFFSET_CAPTURE, $C);
                    [$md, $Og] = $A[0];
                    if (! $md && $q && ! feof($q)) {
                        $H
                        .= fread($q, 1e5);
                        ...
                        if ($md && ! preg_match("(^$Xb)", $md)) {
                            $Ra = driver()->hasCStyleEscapes() || (JUSH == 'pgsql' && ($Og > 0 && strtolower($H[$Og - 1]) == 'e'));
                            $Hg = ($md == '/*' ? '\*/' : ($md == '[' ? ']' : (preg_match("~^$Qe|^#~", $md) ? "\n" : preg_quote($md).($Ra ? '|\\\\.' : ''))));
                            while (preg_match("($Hg|\$)s", $H, $A, PREG_OFFSET_CAPTURE, $C)) {
                                $Dh = $A[0][0];
                                if (! $Dh && $q && ! feof($q)) {
                                    $H
                                    .= fread($q, 1e5);
                            ...
                            $vc = false;
                            $dh = substr($H, 0, $Og + ($Xb[0] == "\n" ? 3 : 0));
                            $nb++;
                            $Xg = "<pre id='sql-$nb'><code class='jush-".JUSH."'>".adminer()->sqlCommandQuery($dh)."</code></pre>\n";
                            if (JUSH == 'sqlite' && preg_match("~^$gi*+ATTACH\\b~i", $dh, $A)) {
                                echo $Xg,"<p class='error'>".'ATTACH queries are not supported.'."\n";
                                $Cc[] = " <a href='#sql-$nb'>$nb</a>";
                                ...
                                if (connection()->multi_query($dh) && $g && preg_match("~^$gi*+USE\\b~i", $dh)) {
                                    ...
                                    if (connection()->error) {
                                        echo ($_POST['only_errors'] ? $Xg : ''),"<p class='error'>".'Error in query'.(connection()->errno ? ' ('.connection()->errno.')' : '').': '.error()."\n";
                                        $Cc[] = " <a href='#sql-$nb'>$nb</a>";
                                        if ($_POST['error_stops']) {
                                            break 2;
                                        }
                                        ...
                                        $Rj = ($_POST['only_errors'] ? '' : driver()->warnings());
                                        $Sj = "warnings-$nb";
                                        if ($Rj) {
                                            $Pi
                                            .= ", <a href='#$Sj'>".'Warnings'.'</a>'.script("qsl('a').onclick = partial(toggle, '$Sj');", '');
                                        }$Kc = null;
                                        $ig = null;
                                        $Lc = "explain-$nb";
                                        if (is_object($I)) {
                                            $z = $_POST['limit'];
                                            $ig = print_select_result($I, $g, [], $z);
                                            if (! $_POST['only_errors']) {
                                                echo "<form action='' method='post'>\n";
                                                $Jf = $I->num_rows;
                                                echo "<p class='sql-footer'>".($Jf ? ($z && $Jf > $z ? sprintf('%d / ', $z) : '').lang_format(['%d row', '%d rows'], $Jf) : ''),$Pi;
                                                if ($g && preg_match("~^($gi|\\()*+SELECT\\b~i", $dh) && ($Kc = explain($g, $dh))) {
                                                    echo ", <a href='#$Lc'>Explain</a>".script("qsl('a').onclick = partial(toggle, '$Lc');", '');
                                                }$t = "export-$nb";
                                            ...
                                            if (preg_match("~^$gi*+(CREATE|DROP|ALTER)$gi++(DATABASE|SCHEMA)\\b~i", $dh)) {
                                                ...
                                                echo "<p class='message' title='".h(connection()->info)."'>".lang_format(['Query executed OK, %d row affected.', 'Query executed OK, %d rows affected.'], $oa)."$Pi\n";
                                            }
                                        }echo $Rj ? "<div id='$Sj' class='hidden'>\n$Rj</div>\n" : '';
                                        if ($Kc) {
                                            echo "<div id='$Lc' class='hidden explain'>\n";
                                            print_select_result($Kc, $g, $ig);
                                            echo "</div>\n";
                                        }
                                    }$oi = microtime(true);
                ...
                echo "<p class='message'>".lang_format(['%d query executed OK.', '%d queries executed OK.'], $nb - count($Cc))," <span class='time'>(".format_time($aj).")</span>\n";
            } elseif ($Cc && $nb > 1) {
                echo "<p class='error'>".'Error in query'.': '.implode('', $Cc)."\n";
            }
        } else {
...
';
    $Ic = "<input type='submit' value='".'Execute'."' title='Ctrl+Enter'>";
    if (! isset($_GET['import'])) {
        $dh = $_GET['sql'];
        if ($_POST) {
            $dh = $_POST['query'];
        ...
        echo '<fieldset><legend>'.'File upload'.'</legend><div>',file_input("SQL$zd: <input type='file' name='sql_file[]' multiple>\n$Ic"),"</div></fieldset>\n";
        $Ud = adminer()->importServerPath();
        if ($Ud) {
            echo '<fieldset><legend>'.'From server'.'</legend><div>',sprintf('Webserver file %s', '<code>'.h($Ud)."$zd</code>"),' <input type="submit" name="webfile" value="'.'Run file'.'">',"</div></fieldset>\n";
        }echo '<p>';
...
';
    ...
    foreach ($n as $B => $m) {
        if ((! $wj && ! isset($m['privileges']['insert'])) || adminer()->fieldName($m) == '') {
            unset($n[$B]);
        }
    ...
    if ($Z) {
        ...
        if (! $Z) {
            ...
            foreach ($K as $x => $X) {
                if (! $Z) {
                    $K[$x] = null;
                }$n[$x] = ['field' => $x, 'null' => ($x != driver()->primary), 'auto_increment' => ($x == driver()->primary)];
            }
        }
    }if ($_POST['save']) {
    ...
    $a = $_GET['create'];
    $Bg = driver()->partitionBy;
    $Eg = ($Bg ? driver()->partitionsInfo($a) : []);
    $mh = referencable_primary($a);
    $kd = [];
    foreach ($mh as $_i => $m) {
        $kd[str_replace('`', '``', $_i).'`'.str_replace('`', '``', $m['field'])] = $_i;
    }$lg = [];
    $S = [];
    if ($a != '') {
        $lg = fields($a);
        ...
        if ($_POST['drop']) {
            ...
            foreach ($K['fields'] as $x => $m) {
                ...
                if ($m['field'] != '') {
                    if (! $m['generated']) {
                        $m['default'] = null;
                    }$ch = process_field($m, $mj);
                    $sa[] = [$m['orig'], $ch, $qa];
                    if (! $kg || $ch !== process_field($kg, $kg)) {
                        $n[] = [$m['orig'], $ch, $qa];
                        if ($m['orig'] != '' || $qa) {
                            $Bj = true;
                        }
                    }if ($p !== null) {
                        $id[idf_escape($m['field'])] = ($a != '' && JUSH != 'sqlite' ? 'ADD' : ' ').format_foreign_key(['table' => $kd[$m['type']], 'source' => [$m['field']], 'target' => [$mj['field']], 'on_delete' => $m['on_delete']]);
            ...
            if (in_array($K['partition_by'], $Bg)) {
                ...
                $E = null;
            }$mf = 'Table has been altered.';
            if ($a == '') {
                cookie('adminer_engine', $K['Engine']);
                $mf = 'Table has been created.';
            }$B = trim($K['name']);
            queries_redirect(ME.(support('table') ? 'table=' : 'select=').urlencode($B), $mf, alter_table($a, $B, (JUSH == 'sqlite' && ($Bj || $id) ? $sa : $n), $id, ($K['Comment'] != $S['Comment'] ? $K['Comment'] : null), ($K['Engine'] && $K['Engine'] != $S['Engine'] ? $K['Engine'] : ''), ($K['Collation'] && $K['Collation'] != $S['Collation'] ? $K['Collation'] : ''), ($K['Auto_increment'] != '' ? number($K['Auto_increment']) : ''), $E));
        }
    }page_header(($a != '' ? 'Alter table' : 'Create table'), $l, ['table' => $a], h($a));
    if (! $_POST) {
        $nj = driver()->types();
        $K = ['Engine' => $_COOKIE['adminer_engine'], 'fields' => [['field' => '', 'type' => (isset($nj['int']) ? 'int' : (isset($nj['integer']) ? 'integer' : '')), 'on_update' => '']], 'partition_names' => ['']];
        if ($a != '') {
            $K = $S;
            $K['name'] = $a;
            $K['fields'] = [];
...
';
    if (support('columns') || $a == '') {
        echo 'Table name'.": <input name='name'".($a == '' && ! $_POST ? ' autofocus' : '')." data-maxlength='64' value='".h($K['name'])."' autocapitalize='off'>\n",($xc ? html_select('Engine', ['' => '('.'engine'.')'] + $xc, $K['Engine']).on_help('event.target.value', 1).script("qsl('select').onchange = helpClose;")."\n" : '');
        if ($jb) {
...
';
    if ($a != '') {
        ...
        $Cg = preg_match('~RANGE|LIST~', $K['partition_by']);
        print_fieldset('partition', 'Partition by', $K['partition_by']);
        echo '<p>'.html_select('partition_by', array_merge([''], $Bg), $K['partition_by']).on_help("event.target.value.replace(/./, 'PARTITION BY \$&')", 1).script("qsl('select').onchange = partitionByChange;"),"(<input name='partition' value='".h($K['partition'])."'>)\n",'Partitions'.": <input type='number' name='partitions' class='size".($Cg || ! $K['partition_by'] ? ' hidden' : '')."' value='".h($K['partitions'])."'>\n","<table id='partition-table'".($Cg ? '' : " class='hidden'").">\n",'<thead><tr><th>'.'Partition name'.'<th>'.'Values'."</thead>\n";
...
';
    ...
    $a = $_GET['indexes'];
    $ce = ['PRIMARY', 'UNIQUE', 'INDEX'];
    $S = table_status1($a, true);
    $Zd = driver()->indexAlgorithms($S);
    ...
    if ($K) {
        ...
        foreach ($K['indexes'] as $v) {
            $B = $v['name'];
            if (in_array($v['type'], $ce)) {
                $e = [];
                $Oe = [];
                $ac = [];
                $ae = (support('partial_indexes') ? $v['partial'] : '');
                $Yd = (in_array($v['algorithm'], $Zd) ? $v['algorithm'] : '');
                $O = [];
                ksort($v['columns']);
                foreach ($v['columns'] as $x => $d) {
                    if ($d != '') {
                ...
                if ($Jc) {
                    ...
                    ksort($Jc['lengths']);
                    ksort($Jc['descs']);
                    if ($v['type'] == $Jc['type'] && array_values($Jc['columns']) === $e && (! $Jc['lengths'] || array_values($Jc['lengths']) === $Oe) && array_values($Jc['descs']) === $ac && $Jc['partial'] == $ae && (! $Zd || $Jc['algorithm'] == $Yd)) {
                        unset($w[$B]);
    ...
    }page_header('Indexes', $l, ['table' => $a], h($a));
    $Xc = array_keys($n);
    if ($_POST['add']) {
...
';
    $Sd = " class='idxopts".($ai ? '' : ' hidden')."'";
    if ($Zd) {
        echo "<th id='label-algorithm'$Sd>".'Algorithm'.doc_link(['sql' => 'create-index.html#create-index-storage-engine-index-types', 'mariadb' => 'storage-engine-index-types/', 'pgsql' => 'indexes-types.html']);
    }echo '<th><input type="submit" class="wayoff">','Columns'.($Oe ? "<span$Sd> (".'length'.')</span>' : '');
    if ($Oe || support('descidx')) {
        echo checkbox('options', 1, $ai, 'Options', 'indexOptionsShow(this.checked)', 'jsonly')."\n";
    }echo '<th id="label-name">Name
';
    if (support('partial_indexes')) {
        echo "<th id='label-condition'$Sd>".'Condition';
...
';
    if ($G) {
        ...
        foreach ($G['columns'] as $x => $d) {
            echo select_input(' disabled', $Xc, $d),"<label><input disabled type='checkbox'>".'descending'.'</label> ';
        }echo "<td><td>\n";
    }$ye = 1;
    foreach ($K['indexes'] as $v) {
        if (! $_POST['drop_col'] || $ye != key($_POST['drop_col'])) {
            echo '<tr><td>'.html_select("indexes[$ye][type]", [-1 => ''] + $ce, $v['type'], ($ye == count($K['indexes']) ? 'indexesAddRow.call(this);' : ''), 'label-type');
            if ($Zd) {
                echo "<td$Sd>".html_select("indexes[$ye][algorithm]", array_merge([''], $Zd), $v['algorithm'], 'label-algorithm');
            }echo '<td>';
            ksort($v['columns']);
            $s = 1;
            foreach ($v['columns'] as $x => $d) {
                echo '<span>'.select_input(" name='indexes[$ye][columns][$s]' title='".'Column'."'", ($n && ($d == '' || $n[$d]) ? array_combine($Xc, $Xc) : []), $d, 'partial('.($s == count($v['columns']) ? 'indexesAddColumn' : 'indexesChangeColumn').", '".js_escape(JUSH == 'sql' ? '' : $_GET['indexes'].'_')."')"),"<span$Sd>",($Oe ? "<input type='number' name='indexes[$ye][lengths][$s]' class='size' value='".h(idx($v['lengths'], $x))."' title='".'Length'."'>" : ''),(support('descidx') ? checkbox("indexes[$ye][descs][$s]", 1, idx($v['descs'], $x), 'descending') : ''),'</span> </span>';
                $s++;
            }echo "<td><input name='indexes[$ye][name]' value='".h($v['name'])."' autocapitalize='off' aria-labelledby='label-name'>\n";
            if (support('partial_indexes')) {
                echo "<td$Sd><input name='indexes[$ye][partial]' value='".h($v['partial'])."' autocapitalize='off' aria-labelledby='label-condition'>\n";
            }echo '<td>'.icon('cross', "drop_col[$ye]", 'x', 'Remove').script("qsl('button').onclick = partial(editingRemoveRow, 'indexes\$1[type]');");
        }$ye++;
    }echo '</table>
</div>
<p>
...
';
    ...
    }page_header(DB != '' ? 'Alter database' : 'Create database', $l, [], h(DB));
    $jb = collations();
    $B = DB;
    if ($_POST) {
        $B = $K['name'];
...
<form action="" method="post">
<p>
',($_POST['add'] || strpos($B, "\n") ? '<textarea autofocus name="name" rows="10" cols="40">'.h($B).'</textarea><br>' : '<input name="name" autofocus value="'.h($B).'" data-maxlength="64" autocapitalize="off">')."\n".($jb ? html_select('collation', ['' => '('.'collation'.')'] + $jb, $K['collation']).doc_link(['sql' => 'charset-charsets.html', 'mariadb' => 'supported-character-sets-and-collations/', 'mssql' => 'relational-databases/system-functions/sys-fn-helpcollations-transact-sql']) : ''),'<input type="submit" value="Save">
';
    if (DB != '') {
...
';
    ...
    foreach ($_h['fields'] as $s => $m) {
        if (substr($m['inout'], -3) == 'OUT' && JUSH == 'sql') {
            $qg[$s] = '@'.idf_escape($m['field']).' AS '.idf_escape($m['field']);
        ...
        foreach ($_h['fields'] as $x => $m) {
            ...
            } elseif (in_array($x, $Vd)) {
                $Sa[] = $X;
            }
        }$H = (isset($_GET['callf']) ? 'SELECT ' : 'CALL ').(idx($_h['returns'], 'type') == 'record' ? '* FROM ' : '').table($ba).'('.implode(', ', $Sa).')';
        $oi = microtime(true);
        $I = connection()->multi_query($H);
        $oa = connection()->affected_rows;
        echo adminer()->selectQuery($H, $oi, ! $I);
        if (! $I) {
            ...
            if ($g) {
                ...
                if (is_object($I)) {
                    ...
                    echo "<p class='message'>".lang_format(['Routine has been called, %d row affected.', 'Routine has been called, %d rows affected.'], $oa)." <span class='time'>".@date('H:i:s')."</span>\n";
                }
            } while (connection()->next_result());
            if ($qg) {
                print_select_result(connection()->query('SELECT '.implode(', ', $qg)));
            }
        }
    }echo '
<form action="" method="post">
...
';
    function pre_tr($Dh)
    {
        return preg_replace('~^~m', '<tr>', preg_replace('~\|~', '<td>', preg_replace('~\|$~m', '', rtrim($Dh))));
    }$R = '(\+--[-+]+\+\n)';
    $K = '(\| .* \|\n)';
...
';
    ...
    }page_header('Foreign key', $l, ['table' => $a], h($a));
    if ($_POST) {
        ksort($K['source']);
        if ($_POST['add']) {
            $K['source'][] = '';
...
';
    ...
    echo '<p><label>'.'Target table'.': '.html_select('table', $lh, $K['table'], $Wf)."</label>\n";
    if (support('scheme')) {
        $Gh = array_filter(adminer()->schemas(), function ($Fh) {
            return ! preg_match('~^information_schema$~i', $Fh);
        });
        echo '<label>'.'Schema'.': '.html_select('ns', $Gh, $K['ns'] != '' ? $K['ns'] : $_GET['ns'], $Wf).'</label>';
        if ($K['ns'] != '') {
            set_schema($mg);
        }
    } elseif (JUSH != 'sqlite') {
        ...
        }echo '<label>'.'DB'.': '.html_select('db', $Qb, $K['db'] != '' ? $K['db'] : $_GET['db'], $Wf).'</label>';
...
';
    $ye = 0;
    foreach ($K['source'] as $x => $X) {
        echo '<tr>','<td>'.html_select('source['.(+$x).']', [-1 => ''] + $fi, $X, ($ye == count($K['source']) - 1 ? 'foreignAddRow.call(this);' : ''), 'label-source'),'<td>'.html_select('target['.(+$x).']', $Ii, idx($K['target'], $x), '', 'label-target');
        $ye++;
    }echo '</table>
<p>
<label>ON DELETE: ',html_select('on_delete', [-1 => ''] + explode('|', driver()->onActions), $K['on_delete']),'</label>
<label>ON UPDATE: ',html_select('on_update', [-1 => ''] + explode('|', driver()->onActions), $K['on_update']),'</label>
...
';
    ...
    if (JUSH == 'pgsql' && $a != '') {
        $P = table_status1($a);
        $ng = strtoupper($P['Engine']);
    }if ($_POST && ! $l) {
        $B = trim($K['name']);
        $wa = " AS\n$K[select]";
        $Se = ME.'table='.urlencode($B);
        $mf = 'View has been altered.';
        $U = ($_POST['materialized'] ? 'MATERIALIZED VIEW' : 'VIEW');
        if (! $_POST['drop'] && $a == $B && JUSH != 'sqlite' && $U == 'VIEW' && $ng == 'VIEW') {
            query_redirect((JUSH == 'mssql' ? 'ALTER' : 'CREATE OR REPLACE').' VIEW '.table($B).$wa, $Se, $mf);
        } else {
            $Ki = $B.'_adminer_'.uniqid();
            drop_create("DROP $ng ".table($a), "CREATE $U ".table($B).$wa, "DROP $U ".table($B), "CREATE $U ".table($Ki).$wa, "DROP $U ".table($Ki), ($_POST['drop'] ? substr(ME, 0, -1) : $Se), 'View has been dropped.', $mf, 'View has been created.', $a, $B);
        }
    }if (! $_POST && $a != '') {
        $K = view($a);
        $K['name'] = $a;
        $K['materialized'] = ($ng != 'VIEW');
        if (! $l) {
            $l = error();
        }
    }page_header(($a != '' ? 'Alter view' : 'Create view'), $l, ['table' => $a], h($a));
    echo '
<form action="" method="post">
...
';
    ...
    $aa = $_GET['event'];
    $pe = ['YEAR', 'QUARTER', 'MONTH', 'DAY', 'HOUR', 'MINUTE', 'WEEK', 'SECOND', 'YEAR_MONTH', 'DAY_HOUR', 'DAY_MINUTE', 'DAY_SECOND', 'HOUR_MINUTE', 'HOUR_SECOND', 'MINUTE_SECOND'];
    $pi = ['ENABLED' => 'ENABLE', 'DISABLED' => 'DISABLE', 'SLAVESIDE_DISABLED' => 'DISABLE ON SLAVE'];
    $K = $_POST;
    if ($_POST && ! $l) {
        if ($_POST['drop']) {
            query_redirect('DROP EVENT '.idf_escape($aa), substr(ME, 0, -1), 'Event has been dropped.');
        } elseif (in_array($K['INTERVAL_FIELD'], $pe) && isset($pi[$K['STATUS']])) {
...
';
    ...
    $K['fields'] = (array) $K['fields'];
    if ($_POST && ! process_fields($K['fields']) && ! $l) {
        $jg = routine($_GET['procedure'], $_h);
        $Ki = "$K[name]_adminer_".uniqid();
        foreach ($K['fields'] as $x => $m) {
            if ($m['field'] == '') {
                unset($K['fields'][$x]);
            }
        }drop_create("DROP $_h ".routine_id($ba, $jg), create_routine($_h, $K), "DROP $_h ".routine_id($K['name'], $K), create_routine($_h, ['name' => $Ki] + $K), "DROP $_h ".routine_id($Ki, $K), substr(ME, 0, -1), 'Routine has been dropped.', 'Routine has been altered.', 'Routine has been created.', $ba, $K['name']);
...
';
    ...
    if (isset($_GET['function'])) {
        ...
        edit_type('returns', (array) $K['returns'], $jb, [], (JUSH == 'pgsql' ? ['void', 'trigger'] : []));
    }echo '</table>
',script('editFields();'),'</div>
<p>';
...
';
    if ($ea != '') {
        $nj = driver()->types();
        $Ac = type_values($nj[$ea]);
        if ($Ac) {
            echo "<code class='jush-".JUSH."'>ENUM (".h($Ac).")</code>\n<p>";
...
';
    ...
    }page_header(($B != '' ? 'Alter check'.': '.h($B) : 'Create check'), $l, ['table' => $a]);
    if (! $K) {
        $ab = driver()->checkConstraints($a);
        $K = ['name' => $B, 'clause' => $ab[$B]];
    }echo '
<form action="" method="post">
<p>';
    if (JUSH != 'sqlite') {
...
';
    ...
    if ($_POST) {
        if (! $l && in_array($_POST['Timing'], $jj['Timing']) && in_array($_POST['Event'], $jj['Event']) && in_array($_POST['Type'], $jj['Type'])) {
            $Tf = ' ON '.table($a);
            $hc = 'DROP TRIGGER '.idf_escape($B).(JUSH == 'pgsql' ? $Tf : '');
            $Se = ME.'table='.urlencode($a);
            if ($_POST['drop']) {
                query_redirect($hc, $Se, 'Trigger has been dropped.');
            } else {
                if ($B != '') {
                    queries($hc);
                }queries_redirect($Se, ($B != '' ? 'Trigger has been altered.' : 'Trigger has been created.'), queries(create_trigger($Tf, $_POST)));
                if ($B != '') {
                    queries(create_trigger($Tf, $K + ['Type' => reset($jj['Type'])]));
                }
            }
        }$K = $_POST;
    }page_header(($B != '' ? 'Alter trigger'.': '.h($B) : 'Create trigger'), $l, ['table' => $a]);
    echo '
<form action="" method="post" id="form">
<table class="layout">
...
';
    ...
    foreach (get_rows('SHOW PRIVILEGES') as $K) {
        foreach (explode(',', ($K['Privilege'] == 'Grant option' ? '' : $K['Context'])) as $_b) {
            $ah[$_b][$K['Privilege']] = $K['Comment'];
        }
    }$ah['Server Admin'] += $ah['File access on server'];
    $ah['Databases']['Create routine'] = $ah['Procedures']['Create routine'];
    unset($ah['Procedures']['Create routine']);
    $ah['Columns'] = [];
    foreach (['Select', 'Insert', 'Update', 'References'] as $X) {
        $ah['Columns'][$X] = $ah['Tables'][$X];
    }unset($ah['Server Admin']['Usage']);
    foreach ($ah['Tables'] as $x => $X) {
        unset($ah['Databases'][$x]);
    }$Bf = [];
    if ($_POST) {
        foreach ($_POST['objects'] as $x => $X) {
            $Bf[$X] = (array) $Bf[$X] + idx($_POST['grants'], $x, []);
        }
    }$ud = [];
    $Rf = '';
    if (isset($_GET['host']) && ($I = connection()->query('SHOW GRANTS FOR '.q($fa).'@'.q($_GET['host'])))) {
        ...
        $Sf = (isset($_GET['host']) ? q($fa).'@'.q($_GET['host']) : "''");
        if ($_POST['drop']) {
            query_redirect("DROP USER $Sf", ME.'privileges=', 'User has been dropped.');
        } else {
            $Df = q($_POST['user']).'@'.q($_POST['host']);
            $Fg = $_POST['pass'];
            if ($Fg != '' && ! $_POST['hashed'] && ! min_version(8)) {
                $Fg = get_val('SELECT PASSWORD('.q($Fg).')');
                $l = ! $Fg;
            }$Eb = false;
            if (! $l) {
                if ($Sf != $Df) {
                    $Eb = queries((min_version(5) ? 'CREATE USER' : 'GRANT USAGE ON *.* TO')." $Df IDENTIFIED BY ".(min_version(8) ? '' : 'PASSWORD ').q($Fg));
                    $l = ! $Eb;
                } elseif ($Fg != $Rf) {
                    queries("SET PASSWORD FOR $Df = ".q($Fg));
                }
            }if (! $l) {
                $xh = [];
                foreach ($Bf as $Lf => $td) {
                    ...
                    if (isset($_GET['grant'])) {
                        $xh = array_diff(array_keys(array_filter($Bf[$Lf], 'strlen')), $td);
                    } elseif ($Sf == $Df) {
                        $Pf = array_keys((array) $ud[$Lf]);
                        $xh = array_diff($Pf, $td);
                        $td = array_diff($td, $Pf);
                        unset($ud[$Lf]);
                    }if (preg_match('~^(.+)\s*(\(.*\))?$~U', $Lf, $A) && (! grant('REVOKE', $xh, $A[2], " ON $A[1] FROM $Df") || ! grant('GRANT', $td, $A[2], " ON $A[1] TO $Df"))) {
                        $l = true;
                        break;
                    }
                }
                ...
                if ($Sf != $Df) {
                    queries("DROP USER $Sf");
                } elseif (! isset($_GET['grant'])) {
                    foreach ($ud as $Lf => $xh) {
                        if (preg_match('~^(.+)(\(.*\))?$~U', $Lf, $A)) {
                            grant('REVOKE', array_keys($xh), $A[2], " ON $A[1] FROM $Df");
                        }
                    }
                }
            }queries_redirect(ME.'privileges=', (isset($_GET['host']) ? 'User has been altered.' : 'User has been created.'), ! $l);
            if ($Eb) {
                connection()->query("DROP USER $Df");
            }
        }
    }page_header((isset($_GET['host']) ? 'Username'.': '.h("$fa@$_GET[host]") : 'Create user'), $l, ['privileges' => ['', 'Privileges']]);
    $K = $_POST;
    if ($K) {
        $ud = $Bf;
    } else {
        ...
        if ($Rf != '') {
            $K['hashed'] = true;
        }$ud[(DB == '' || $ud ? '' : idf_escape(addcslashes(DB, '%_\\'))).'.*'] = [];
    }echo '<form action="" method="post">
<table class="layout">
...
',"<table class='odds'>\n","<thead><tr><th colspan='2'>".'Privileges'.doc_link(['sql' => 'grant.html#priv_level']);
    $s = 0;
    foreach ($ud as $Lf => $td) {
        echo '<th>'.($Lf != '*.*' ? "<input name='objects[$s]' value='".h($Lf)."' size='10' autocapitalize='off'>" : input_hidden("objects[$s]", '*.*').'*.*');
        $s++;
    }echo "</thead>\n";
    foreach (['' => '', 'Server Admin' => 'Server', 'Databases' => 'Database', 'Tables' => 'Table', 'Columns' => 'Column', 'Procedures' => 'Routine'] as $_b => $Yb) {
        foreach ((array) $ah[$_b] as $Zg => $ob) {
            echo '<tr><td'.($Yb ? ">$Yb<td" : " colspan='2'").' lang="en" title="'.h($ob).'">'.h($Zg);
            $s = 0;
            foreach ($ud as $Lf => $td) {
                $B = "'grants[$s][".h(strtoupper($Zg))."]'";
                $Y = $td[strtoupper($Zg)];
                if ($_b == 'Server Admin' && $Lf != (isset($ud['*.*']) ? '*.*' : '.*')) {
                    echo '<td>';
                } elseif (isset($_GET['grant'])) {
                    echo "<td><select name=$B><option><option value='1'".($Y ? ' selected' : '').'>'.'Grant'."<option value='0'".($Y == '0' ? ' selected' : '').'>'.'Revoke'.'</select>';
                } else {
                    echo "<td align='center'><label class='block'>","<input type='checkbox' name=$B value='1'".($Y ? ' checked' : '').($Zg == 'All privileges' ? " id='grants-$s-all'>" : '>'.($Zg == 'Grant option' ? '' : script("qsl('input').onclick = function () { if (this.checked) formUncheck('grants-$s-all'); };"))),'</label>';
                }$s++;
            }
        }
    }echo "</table>\n",'<p>
...
',script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});");
    ...
    foreach (adminer()->processList() as $s => $K) {
        if (! $s) {
            ...
            foreach ($K as $x => $X) {
                echo "<th>$x".doc_link(['sql' => 'show-processlist.html#processlist_'.strtolower($x), 'pgsql' => 'monitoring-stats.html#PG-STAT-ACTIVITY-VIEW', 'oracle' => 'REFRN30223']);
            }echo "</thead>\n";
...
',script('tableCheck();');
    ...
    foreach ($n as $x => $m) {
        ...
        $_f = html_entity_decode(strip_tags($B), ENT_QUOTES);
    ...
    foreach ($w as $v) {
        ...
        }$Uj = ($Uj ? "\nWHERE ".implode(' AND ', $Uj) : '');
        if ($_POST['export']) {
            save_settings(['output' => $_POST['output'], 'format' => $_POST['format']], 'adminer_import');
            dump_headers($a);
            adminer()->dumpTable($a, '');
            $od = ($M ? implode(', ', $M) : '*').convert_fields($e, $n, $M)."\nFROM ".table($a);
            $xd = ($vd && $te ? "\nGROUP BY ".implode(', ', $vd) : '').($eg ? "\nORDER BY ".implode(', ', $eg) : '');
            $H = "SELECT $od$Uj$xd";
            if (is_array($_POST['check']) && ! $G) {
                $qj = [];
                foreach ($_POST['check'] as $X) {
                    $qj[] = '(SELECT'.limit($od, "\nWHERE ".($Z ? implode(' AND ', $Z).' AND ' : '').where_check($X, $n).$xd, 1).')';
            ...
            if ($_POST['save'] || $_POST['delete']) {
                ...
                if (! $_POST['delete']) {
                    ...
                    if ($_POST['all'] || ($G && is_array($_POST['check'])) || $te) {
                        $I = ($_POST['delete'] ? driver()->delete($a, $Uj) : ($_POST['clone'] ? queries("INSERT $H$Uj".driver()->insertReturning($a)) : driver()->update($a, $O, $Uj)));
                        $oa = connection()->affected_rows;
                        if (is_object($I)) {
                            $oa += $I->num_rows;
                        }
                        ...
                        foreach ((array) $_POST['check'] as $X) {
                            ...
                            $I = ($_POST['delete'] ? driver()->delete($a, $Tj, 1) : ($_POST['clone'] ? queries('INSERT'.limit1($a, $H, $Tj)) : driver()->update($a, $O, $Tj, 1)));
                            if (! $I) {
                                break;
                            }$oa += connection()->affected_rows;
                        }
                    }
                }$mf = lang_format(['%d item has been affected.', '%d items have been affected.'], $oa);
                if ($_POST['clone'] && $I && $oa == 1) {
                    $Je = last_id($I);
                    if ($Je) {
                        $mf = sprintf('Item%s has been inserted.', " $Je");
                    }
                }queries_redirect(remove_from_uri($_POST['all'] && $_POST['delete'] ? 'page' : ''), $mf, $I);
                if (! $_POST['delete']) {
                    $Rg = (array) $_POST['fields'];
                    edit_form($a, array_intersect_key($n, $Rg), $Rg, ! $_POST['clone'], $l);
                    page_footer();
                    exit;
                }
            } elseif (! $_POST['import']) {
                if (! $_POST['val']) {
                    ...
                    foreach ($_POST['val'] as $sj => $K) {
                        ...
                        }$I = driver()->update($a, $O, ' WHERE '.($Z ? implode(' AND ', $Z).' AND ' : '').where_check($sj, $n), ($te || $G ? 0 : 1), ' ');
                        if (! $I) {
                            break;
                        }$oa += connection()->affected_rows;
                    }queries_redirect(remove_from_uri(), lang_format(['%d item has been affected.', '%d items have been affected.'], $oa), $I);
                }
            } elseif (! is_string($Yc = get_file('csv_file', true))) {
                $l = upload_error($Yc);
            } elseif (! preg_match('~~u', $Yc)) {
                $l = 'File must be in UTF-8 encoding.';
            } else {
                save_settings(['output' => $na['output'], 'format' => $_POST['separator']], 'adminer_import');
                $I = true;
                $kb = array_keys($n);
                preg_match_all('~(?>"[^"]*"|[^"\r\n]+)+~', $Yc, $Ze);
                $oa = count($Ze[0]);
                ...
                if ($I) {
                    driver()->commit();
                }queries_redirect(remove_from_uri('page'), lang_format(['%d row has been imported.', '%d rows have been imported.'], $oa), $I);
                driver()->rollback();
            }
        }
    ...
    if (isset($yh['insert']) || ! support('table')) {
        ...
        foreach ((array) $_GET['where'] as $X) {
            if (isset($kd[$X['col']]) && count($kd[$X['col']]) == 1 && ($X['op'] == '=' || (! $X['op'] && (is_array($X['val']) || ! preg_match('~[_%]~', $X['val']))))) {
                $wg['set'.'['.bracket_escape($X['col']).']'] = $X['val'];
            }
        }$O = $wg ? '&'.http_build_query($wg) : '';
    ...
    if (! $e && support('table')) {
        ...
        adminer()->selectOrderPrint($eg, $fg, $w);
        ...
        $nd = null;
        if ($D == 'last') {
            $nd = get_val(count_rows($a, $Z, $te, $vd));
            $D = floor(max(0, intval($nd) - 1) / $z);
        }$Mh = $M;
        $wd = $vd;
        if (! $Mh) {
            $Mh[] = '*';
        ...
        }$I = driver()->select($a, $Mh, $Z, $wd, $eg, $z, $D, true);
        if (! $I) {
            echo "<p class='error'>".error()."\n";
        } else {
            if (JUSH == 'mssql' && $D) {
            ...
            while ($K = $I->fetch_assoc()) {
            ...
            }if ($_GET['page'] != 'last' && $z && $vd && $te && JUSH == 'sql') {
                $nd = get_val(' SELECT FOUND_ROWS()');
            }if (! $L) {
                echo "<p class='message'>".'No rows.'."\n";
            } else {
                $Ea = adminer()->backwardKeys($a, $_i);
                echo "<div class='scrollable'>","<table id='table' class='nowrap checkable odds'>",script("mixin(qs('#table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true), onkeydown: editingKeydown});"),'<thead><tr>'.(! $vd && $M ? '' : "<td><input type='checkbox' id='all-page' class='jsonly'>".script("qs('#all-page').onclick = partial(formCheck, /check/);", '')." <a href='".h($_GET['modify'] ? remove_from_uri('modify') : $_SERVER['REQUEST_URI'].'&modify=1')."'>".'Modify'.'</a>');
                $Af = [];
                $qd = [];
                reset($M);
                $ih = 1;
                foreach ($L[0] as $x => $X) {
                    if (! isset($uj[$x])) {
                        ...
                        if ($B != '') {
                            ...
                            $Af[$x] = $B;
                            $d = idf_escape($x);
                            $Md = remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($x);
                            $Yb = '&desc%5B0%5D=1';
                            ...
                            $pd = apply_sql_function($X['fun'], $B);
                            $ei = isset($m['privileges']['order']) || $pd != $B;
                            echo $ei ? "<a href='".h($Md.($eg[0] == $d || $eg[0] == $x ? $Yb : ''))."'>$pd</a>" : $pd;
                            $lf = ($ei ? "<a href='".h($Md.$Yb)."' title='".'descending'."' class='text'> â†“</a>" : '');
                            if (! $X['fun'] && isset($m['privileges']['where'])) {
                                $lf
                                .= '<a href="#fieldset-search" title="'.'Search'.'" class="text jsonly"> =</a>';
                                $lf
                                .= script("qsl('a').onclick = partial(selectSearch, '".js_escape($x)."');");
                            }echo $lf ? "<span class='column hidden'>$lf</span>" : '';
                        }$qd[$x] = $X['fun'];
                        next($M);
                    }
                }$Oe = [];
                ...
                if (is_ajax()) {
                    ob_end_clean();
                }foreach (adminer()->rowDescriptions($L, $kd) as $zf => $K) {
                    $rj = unique_array($L[$zf], $w);
                    if (! $rj) {
                        $rj = [];
                        reset($M);
                        foreach ($L[$zf] as $x => $X) {
                    ...
                    }echo '<tr>'.(! $vd && $M ? '' : '<td>'.checkbox('check[]', substr($sj, 1), in_array(substr($sj, 1), (array) $_POST['check'])).($te || information_schema(DB) ? '' : " <a href='".h(ME.'edit='.urlencode($a).$sj)."' class='edit'>".'edit'.'</a>'));
                    reset($M);
                    foreach ($K as $x => $X) {
                        if (isset($Af[$x])) {
                            $d = current($M);
                            $m = (array) $n[$x];
                            if ($X != '' && (! isset($uc[$x]) || $uc[$x] != '')) {
                                $uc[$x] = (is_mail($X) ? $Af[$x] : '');
                            }$_ = '';
                            if (is_blob($m) && $X != '') {
                                $_ = ME.'download='.urlencode($a).'&field='.urlencode($x).$sj;
                            }if (! $_ && $X !== null) {
                                foreach ((array) $kd[$x] as $p) {
                                    if (count($kd[$x]) == 1 || end($p['source']) == $x) {
                                        $_ = '';
                                        foreach ($p['source'] as $s => $fi) {
                                            $_
                                            .= where_link($s, $p['target'][$s], $L[$zf][$fi]);
                            ...
                            }$Nd = select_value($X, $_, $m, $Oi);
                            $t = h("val[$sj][".bracket_escape($x).']');
                            $Sg = idx(idx($_POST['val'], $sj), bracket_escape($x));
                            $pc = ! is_array($K[$x]) && is_utf8($Nd) && $L[$zf][$x] == $K[$x] && ! $qd[$x] && ! $m['generated'];
                            $U = (preg_match('~^(AVG|MIN|MAX)\((.+)\)~', $d, $A) ? $n[idf_unescape($A[2])]['type'] : $m['type']);
                            $Mi = preg_match('~text|json|lob~', $U);
                            $ue = preg_match(number_type(), $U) || preg_match('~^(CHAR_LENGTH|ROUND|FLOOR|CEIL|TIME_TO_SEC|COUNT|SUM)\(~', $d);
                            echo "<td id='$t'".($ue && ($X === null || is_numeric(strip_tags($Nd)) || $U == 'money') ? " class='number'" : '');
                            if (($_GET['modify'] && $pc && $X !== null) || $Sg !== null) {
                                $_d = h($Sg !== null ? $Sg : $K[$x]);
                                echo '>'.($Mi ? "<textarea name='$t' cols='30' rows='".(substr_count($K[$x], "\n") + 1)."'>$_d</textarea>" : "<input name='$t' value='$_d' size='$Oe[$x]'>");
                            } else {
                                $Ue = strpos($Nd, '<i>â€¦</i>');
                                echo " data-text='".($Ue ? 2 : ($Mi ? 1 : 0))."'".($pc ? '' : " data-warning='".h('Use edit link to modify this value.')."'").">$Nd";
                            }
                        }next($M);
                    }if ($Ea) {
                        echo '<td>';
                    }adminer()->backwardKeysPrint($Ea, $L[$zf]);
                    echo "</tr>\n";
                }if (is_ajax()) {
                    exit;
                }echo "</table>\n","</div>\n";
                ...
                if ($L || $D) {
                    ...
                    if ($_GET['page'] != 'last') {
                        if (! $z || (count($L) < $z && ($L || ! $D))) {
                            ...
                            $nd = ($te ? false : found_rows($S, $Z));
                            if (intval($nd) < max(1e4, 2 * ($D + 1) * $z)) {
                                $nd = first(slow_query(count_rows($a, $Z, $te, $vd)));
                            } elseif (JUSH == 'sql' || JUSH == 'pgsql') {
                                $Hc = false;
                            }
                        }
                    }$ug = ($z && ($nd === false || $nd > $z || $D));
                    if ($ug) {
                        echo (($nd === false ? count($L) + 1 : $nd - $D * $z) > $z ? '<p><a href="'.h(remove_from_uri('page').'&page='.($D + 1)).'" class="loadmore">'.'Load more data'.'</a>'.script("qsl('a').onclick = partial(selectLoadMore, $z, '".'Loading'."â€¦');", '') : ''),"\n";
                    }echo "<div class='footer'><div>\n";
                    if ($ug) {
                        $ef = ($nd === false ? $D + (count($L) >= $z ? 2 : 1) : floor(($nd - 1) / $z));
                        echo '<fieldset>';
                        if (JUSH != 'simpledb') {
                            echo "<legend><a href='".h(remove_from_uri('page'))."'>".'Page'.'</a></legend>',script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".'Page'."', '".($D + 1)."')); return false; };"),pagination(0, $D).($D > 5 ? ' â€¦' : '');
                            for ($s = max(1, $D - 4); $s < min($ef, $D + 5); $s++) {
                                echo pagination($s, $D);
                            }if ($ef > 0) {
                                echo ($D + 5 < $ef ? ' â€¦' : ''),($Hc && $nd !== false ? pagination($ef, $D) : " <a href='".h(remove_from_uri('page').'&page=last')."' title='~$ef'>".'last'.'</a>');
                            }
                        } else {
                            echo '<legend>'.'Page'.'</legend>',pagination(0, $D).($D > 1 ? ' â€¦' : ''),($D ? pagination($D, $D) : ''),($ef > $D ? pagination($D + 1, $D).($ef > $D + 1 ? ' â€¦' : '') : '');
                        }echo "</fieldset>\n";
                    }echo '<fieldset>','<legend>'.'Whole result'.'</legend>';
                    $ec = ($Hc ? '' : '~ ').$nd;
                    $Xf = "const checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$ec' : checked); selectCount('selected2', this.checked || !checked ? '$ec' : checked);";
                    echo checkbox('all', 1, 0, ($nd !== false ? ($Hc ? '' : '~ ').lang_format(['%d row', '%d rows'], $nd) : ''), $Xf)."\n","</fieldset>\n";
...
';
                    ...
                    foreach ((array) $_GET['columns'] as $d) {
                        ...
                        $rg = adminer()->dumpOutput();
                        echo ($rg ? html_select('output', $rg, $na['output']).' ' : ''),html_select('format', $ld, $na['format'])," <input type='submit' name='export' value='".'Export'."'>\n","</div></fieldset>\n";
                    }adminer()->selectEmailPrint(array_filter($uc, 'strlen'), $e);
                    echo "</div></div>\n";
                }if (adminer()->selectImportPrint()) {
                    echo '<p>',"<a href='#import'>".'Import'.'</a>',script("qsl('a').onclick = partial(toggle, 'import');", ''),"<span id='import'".($_POST['import'] ? '' : " class='hidden'").'>: ',file_input("<input type='file' name='csv_file'> ".html_select('separator', ['csv' => 'CSV,', 'csv;' => 'CSV;', 'tsv' => 'TSV'], $na['format'])." <input type='submit' name='import' value='".'Import'."'>"),'</span>';
    ...
    $Kj = ($P ? adminer()->showStatus() : adminer()->showVariables());
    if (! $Kj) {
        echo "<p class='message'>".'No rows.'."\n";
    } else {
        echo "<table>\n";
    ...
    header('Content-Type: text/javascript; charset=utf-8');
    if ($_GET['script'] == 'db') {
        $wi = ['Data_length' => 0, 'Index_length' => 0, 'Data_free' => 0];
        foreach (table_status() as $B => $S) {
            json_row("Comment-$B", h($S['Comment']));
            if (! is_view($S) || preg_match('~materialized~i', $S['Engine'])) {
                foreach (['Engine', 'Collation'] as $x) {
                    json_row("$x-$B", h($S[$x]));
                }foreach ($wi + ['Auto_increment' => 0, 'Rows' => 0] as $x => $X) {
                    if ($S[$x] != '') {
                        $X = format_number($S[$x]);
                        if ($X >= 0) {
    ...
    if ($Gi && ! $l && ! $_POST['search']) {
        ...
        }queries_redirect(substr(ME, 0, -1), $mf, $I);
    ...
    if (adminer()->homepage()) {
        if ($_GET['ns'] !== '') {
            ...
            if (! $Fi) {
                ...
                }echo "<div class='scrollable'>\n","<table class='nowrap checkable odds'>\n",script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});"),'<thead><tr class="wrap">','<td><input id="check-all" type="checkbox" class="jsonly">'.script("qs('#check-all').onclick = partial(formCheck, /^(tables|views)\[/);", ''),'<th>'.'Table';
                $e = ['Engine' => ['Engine'.doc_link(['sql' => 'storage-engines.html'])]];
                if (collations()) {
                    $e['Collation'] = ['Collation'.doc_link(['sql' => 'charset-charsets.html', 'mariadb' => 'supported-character-sets-and-collations/'])];
                }if (function_exists('Adminer\alter_table')) {
                    $e['Data_length'] = ['Data Length'.doc_link(['sql' => 'show-table-status.html', 'pgsql' => 'functions-admin.html#FUNCTIONS-ADMIN-DBOBJECT', 'oracle' => 'REFRN20286']), 'create', 'Alter table'];
                }if (support('indexes')) {
                    $e['Index_length'] = ['Index Length'.doc_link(['sql' => 'show-table-status.html', 'pgsql' => 'functions-admin.html#FUNCTIONS-ADMIN-DBOBJECT']), 'indexes', 'Alter indexes'];
                }$e['Data_free'] = ['Data Free'.doc_link(['sql' => 'show-table-status.html']), 'edit', 'New item'];
                if (function_exists('Adminer\alter_table')) {
                    $e['Auto_increment'] = ['Auto Increment'.doc_link(['sql' => 'example-auto-increment.html', 'mariadb' => 'auto_increment/']), 'auto_increment=1&create', 'Alter table'];
                }$e['Rows'] = ['Rows'.doc_link(['sql' => 'show-table-status.html', 'pgsql' => 'catalog-pg-class.html#CATALOG-PG-CLASS', 'oracle' => 'REFRN20286']), 'select', 'Select data'];
                if (support('comment')) {
                    $e['Comment'] = ['Comment'.doc_link(['sql' => 'show-table-status.html', 'pgsql' => 'functions-info.html#FUNCTIONS-INFO-COMMENT-TABLE'])];
                }foreach ($e as $d) {
                    echo "<td>$d[0]";
                }echo "</thead>\n";
                $T = 0;
                foreach ($Fi as $B => $U) {
                    $Nj = ($U !== null && ! preg_match('~table|sequence~i', $U));
                    $t = h('Table-'.$B);
                    echo '<tr><td>'.checkbox(($Nj ? 'views[]' : 'tables[]'), $B, in_array("$B", $Gi, true), '', '', '', $t),'<th>'.(support('table') || support('indexes') ? "<a href='".h(ME).'table='.urlencode($B)."' title='".'Show structure'."' id='$t'>".h($B).'</a>' : h($B));
                    if ($Nj && ! preg_match('~materialized~i', $U)) {
                        $Si = 'View';
                        ...
                        foreach ($e as $x => $d) {
                            $t = " id='$x-".h($B)."'";
                            echo $d[1] ? "<td align='right'><a href='".h(ME."$d[1]=").urlencode($B)."'$t title='$d[2]'>?</a>" : "<td id='$x-".h($B)."'>";
                        }$T++;
                    }echo "\n";
            ...
            if (support('routine')) {
                ...
                if ($L) {
                    ...
                    $Fc = get_val('SELECT @@event_scheduler');
                    if ($Fc && $Fc != 'ON') {
                        echo "<p class='error'><code class='jush-sqlset'>event_scheduler</code>: ".h($Fc)."\n";
                    }
